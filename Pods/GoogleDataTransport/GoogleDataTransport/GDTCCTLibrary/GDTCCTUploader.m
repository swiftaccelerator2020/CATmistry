...
/** */
static NSString *const kLibraryDataCCTNextUploadTimeKey =
...
#if !NDEBUG
NSNotificationName const GDTCCTUploadCompleteNotification =
    @"com.GDTCCTUploader.UploadComplete";
#endif // #if !NDEBUG

typedef void (^GDTCCTUploaderURLTaskCompletion)(
    NSNumber *batchID, NSSet<GDTCOREvent *> *_Nullable events,
    NSData *_Nullable data, NSURLResponse *_Nullable response,
    NSError *_Nullable error);

typedef void (^GDTCCTUploaderEventBatchBlock)(
    NSNumber *_Nullable batchID, NSSet<GDTCOREvent *> *_Nullable events);
...
/// A flag indicating if there is an ongoing upload. The current implementation
/// supports only a single upload operation. If `uploadTarget` method is called
/// when  `isCurrentlyUploading == YES` then no new uploads will be started.
...
+ (void)load {
    ...
    [[GDTCORRegistrar sharedInstance] registerUploader:uploader
...
- (instancetype)init {
    self = [super init];
    if (self) {
        _uploaderQueue = dispatch_queue_create("com.google.GDTCCTUploader",
...
- (nullable NSURL *)serverURLForTarget:(GDTCORTarget)target {
    // These strings should be interleaved to construct the real URL. This is just
    // to (hopefully) fool github URL scanning bots.
    ...
    dispatch_once(&CCTOnceToken, ^ {
        ...
        const char URL[54] = {
            p1[0],  p2[0],  p1[1],  p2[1],  p1[2],  p2[2],  p1[3],  p2[3],  p1[4],
    ...
    dispatch_once(&CSHOnceToken, ^ {
        // These strings should be interleaved to construct the real URL. This is
        // just to (hopefully) fool github URL scanning bots.
...
- (void)uploadTarget:(GDTCORTarget)target
    ...
    __block GDTCORBackgroundIdentifier backgroundTaskID =
             ...
             NSSet<GDTCOREvent *> *_Nullable events) {
                 ...
                 if (!events || events.count == 0) {
                     dispatch_async(self.uploaderQueue, ^ {
                         GDTCORLogDebug(@"Target %ld reported as ready for "
                                   @"upload, but no "
...
- (void)uploadBatchWithID:(nullable NSNumber *)batchID
    ...
    [self sendURLRequestForBatchWithID:batchID
          events:events
          target:target
         ...
         NSError *_Nullable error) {
             dispatch_async(self.uploaderQueue, ^ {
                 [self handleURLResponse:response
                  ...
                  batchID:batchID];
#if !NDEBUG
                 // Post a notification when in DEBUG mode to state how
                 // many packages were uploaded. Useful for validation
                 // during tests.
...
/** Validates events and sends URL request and calls completion with the result.
 * Modifies uploading state in the case of the failure.*/
- (void)sendURLRequestForBatchWithID:(nullable NSNumber *)batchID
    events:(nullable NSSet<GDTCOREvent *> *)events
    target:(GDTCORTarget)target
    completionHandler:
    (GDTCCTUploaderURLTaskCompletion)completionHandler {
    dispatch_async(self.uploaderQueue, ^ {
        ...
        GDTCORLogDebug(@"CTT: request containing %lu events created: %@",
                       (unsigned long)events.count, request);
...
- (void)handleURLResponse:(nullable NSURLResponse *)response
    ...
    if (error) {
        GDTCORLogWarning(GDTCORMCWUploadFailed,
                         @"There was an error uploading events: %@", error);
    }
    NSError *decodingError;
    GDTCORClock *futureUploadTime;
    if (data) {
        gdt_cct_LogResponse logResponse =
            GDTCCTDecodeLogResponse(data, &decodingError);
        if (!decodingError && logResponse.has_next_request_wait_millis) {
            GDTCORLogDebug(@"CCT: The backend responded asking to not upload for "
                           @"%lld millis from now.",
                           logResponse.next_request_wait_millis);
            futureUploadTime = [GDTCORClock
                                clockSnapshotInTheFuture:logResponse.next_request_wait_millis];
    ...
    if (error || ((NSHTTPURLResponse *)response).statusCode == 429 ||
        ...
        // Move the events back to the main storage to be uploaded on the next
        // attempt.
...
/** Fetches a batch of pending events for the specified target and conditions.
 * Passes `nil` to completion if there are no suitable events to upload. */
- (void)batchToUploadForTarget:(GDTCORTarget)target
    ...
    [self removeBatchesForTarget:target
          storage:storage
         onComplete:^ {
             // There may be a big amount of events stored, so creating
             // a batch may be an expensive operation.

             // 3. Do a lightweight check if there are any events for
             // the target first to finish early if there are no.
             [storage hasEventsForTarget:target
             onComplete:^(BOOL hasEvents) {
                 // 4. Proceed with fetching the events.
                 [self
                  batchToUploadForTarget:target
                  storage:storage
                  conditions:conditions
                  hasEvents:hasEvents
...
- (void)batchToUploadForTarget:(GDTCORTarget)target
    ...
    dispatch_async(self.uploaderQueue, ^ {
        ...
        // All conditions have been checked and met. Lock uploader for this target
        // to prevent other targets upload attempts.
        self.isCurrentlyUploading = YES;

        // Fetch a batch to upload and pass along.
        GDTCORStorageEventSelector *eventSelector =
        [self eventSelectorTarget:target withConditions:conditions];
...
- (void)removeBatchesForTarget:(GDTCORTarget)target
    ...
    [storage batchIDsForTarget:target
        ...
        // Wait until all batches are removed and call completion
        // handler.
...
- (BOOL)readyToUploadTarget:(GDTCORTarget)target
    ...
    if (self.isCurrentlyUploading) {
        GDTCORLogDebug(@"%@", @"CCT: Wait until previous upload finishes. The "
    ...
    // See https://developer.apple.com/videos/play/wwdc2019/712/ (49:40) for more
    // details.
    if (conditions & GDTCORUploadConditionNoNetwork) {
        GDTCORLogDebug(@"%@",
                       @"CCT: Not ready to upload without a network connection.");
        return NO;
    }

    // Upload events when there are with no additional conditions for
    // kGDTCORTargetCSH.
    ...
    if ((conditions & GDTCORUploadConditionHighPriority) ==
            GDTCORUploadConditionHighPriority) {
    ...
    case kGDTCORTargetCCT:
        if (self->_CCTNextUploadTime) {
            isAfterNextUploadTime =
                [[GDTCORClock snapshot] isAfter:self->_CCTNextUploadTime];
        }
        break;
    ...
    case kGDTCORTargetFLL:
        if (self->_FLLNextUploadTime) {
            isAfterNextUploadTime =
                [[GDTCORClock snapshot] isAfter:self->_FLLNextUploadTime];
        }
        break;
    ...
    if (isAfterNextUploadTime) {
        GDTCORLogDebug(@"CCT: can upload to target %ld because the request wait "
                       @"time has transpired",
                       (long)target);
    } else {
        GDTCORLogDebug(
            @"CCT: can't upload to target %ld because the backend asked to wait",
            (long)target);
    }
 ...
 */
- (nonnull NSData *)constructRequestProtoWithEvents:
    (NSSet<GDTCOREvent *> *)events {
    // Segment the log events by log type.
    NSMutableDictionary<NSString *, NSMutableSet<GDTCOREvent *> *>
    *logMappingIDToLogSet = [[NSMutableDictionary alloc] init];
    [events enumerateObjectsUsingBlock:^(GDTCOREvent *_Nonnull event,
           BOOL *_Nonnull stop) {
 ...
 */
- (nullable NSURLRequest *)constructRequestForTarget:(GDTCORTarget)target
    data:(NSData *)data {
    if (data == nil || data.length == 0) {
        GDTCORLogDebug(@"There was no data to construct a request for target %ld.",
                       (long)target);
        return nil;
    }
    ...
    NSString *userAgent = [NSString
                           stringWithFormat:@"datatransport/%@ %@support/%@ apple/", kGDTCORVersion,
                           targetString, kGDTCCTSupportSDKVersion];
    if (target == kGDTCORTargetFLL || target == kGDTCORTargetCSH) {
        [request setValue:[self FLLAndCSHandINTAPIKey]
...
/** */
- (nullable GDTCORStorageEventSelector *)
    eventSelectorTarget:(GDTCORTarget)target
    withConditions:(GDTCORUploadConditions)conditions {
    id<GDTCORStorageProtocol> storage = GDTCORStorageInstanceForTarget(target);
    if ((conditions & GDTCORUploadConditionHighPriority) ==
            GDTCORUploadConditionHighPriority) {
    ...
    NSString *lastDailyUploadDataKey =
        [NSString stringWithFormat:@"%@LastDailyUpload-%ld",
                  NSStringFromClass([self class]), (long)target];
    [storage libraryDataForKey:lastDailyUploadDataKey
            ...
            return [NSData dataWithBytes:&lastDayOfDailyUpload
...
- (void)appWillForeground:(GDTCORApplication *)app {
    dispatch_async(_uploaderQueue, ^ {
        ...
        kWeekday = [gregorianCalendar component:NSCalendarUnitWeekday
                                      fromDate:date];
    });
}
