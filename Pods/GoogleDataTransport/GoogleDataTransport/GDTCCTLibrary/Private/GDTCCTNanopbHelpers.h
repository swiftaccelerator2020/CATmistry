 ...
 *
 * @note calloc is called in this method. Ensure that pb_release is called on
 * this or the parent.
 *
 * @param string The string to convert.
 * @return A newly allocated array of bytes representing the UTF8 encoding of
 * the string.
 */
...
FOUNDATION_EXPORT
NSData *
GDTCCTEncodeBatchedLogRequest(gdt_cct_BatchedLogRequest *batchedLogRequest);

/** Constructs a gdt_cct_BatchedLogRequest given sets of events segemented by
 * mapping ID.
 *
 * @note calloc is called in this method. Ensure that pb_release is called on
 * this or the parent.
 *
 * @param logMappingIDToLogSet A map of mapping IDs to sets of events to convert
 * into a batch.
...
/** Constructs the data of a gdt_cct_NetworkConnectionInfo representing the
 * client nework connection information.
 *
...
GDTCCTNetworkConnectionInfoNetworkMobileSubtype(void);
 ...
 * @param error An error that will be populated if something went wrong during
 * decoding.
 * @return A newly allocated gdt_cct_LogResponse from the data, if the bytes
 * decoded properly.
 */
FOUNDATION_EXPORT
