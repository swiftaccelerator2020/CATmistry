...
static NSString *const gBatchIDCounterKey =
...
NSString *const kGDTCOREventComponentsEventIDKey =
    @"GDTCOREventComponentsEventIDKey";

NSString *const kGDTCOREventComponentsQoSTierKey =
    @"GDTCOREventComponentsQoSTierKey";

NSString *const kGDTCOREventComponentsMappingIDKey =
    @"GDTCOREventComponentsMappingIDKey";

NSString *const kGDTCOREventComponentsExpirationKey =
    @"GDTCOREventComponentsExpirationKey";

NSString *const kGDTCORBatchComponentsTargetKey =
    @"GDTCORBatchComponentsTargetKey";

NSString *const kGDTCORBatchComponentsBatchIDKey =
    @"GDTCORBatchComponentsBatchIDKey";

NSString *const kGDTCORBatchComponentsExpirationKey =
    @"GDTCORBatchComponentsExpirationKey";

@implementation GDTCORFlatFileStorage

+ (void)load {
#if !NDEBUG
    [[GDTCORRegistrar sharedInstance] registerStorage:[self sharedInstance]
    ...
    [[GDTCORRegistrar sharedInstance] registerStorage:[self sharedInstance]
                                      target:kGDTCORTargetINT];

    // Sets a global translation mapping to decode GDTCORStoredEvent objects
    // encoded as instances of GDTCOREvent instead. Then we do the same thing with
    // GDTCORStorage. This must be done in load because there are no direct
    // references to this class and the NSCoding methods won't be called unless
    // the class name is mapped early.
    [NSKeyedUnarchiver setClass:[GDTCOREvent class]
                       forClassName:@"GDTCORStoredEvent"];
    [NSKeyedUnarchiver setClass:[GDTCORFlatFileStorage class]
                       forClassName:@"GDTCORStorage"];
}
...
- (instancetype)init {
    self = [super init];
    if (self) {
        _storageQueue = dispatch_queue_create("com.google.GDTCORFlatFileStorage",
...
- (void)storeEvent:(GDTCOREvent *)event
    ...
    if (!completion) {
        completion = ^(BOOL wasWritten, NSError *_Nullable error) {
            GDTCORLogDebug(@"event %@ stored. success:%@ error:%@", event,
                           wasWritten ? @"YES" : @"NO", error);
        };
    }
    ...
    dispatch_async(_storageQueue, ^ {
        ...
        NSString *filePath =
        [GDTCORFlatFileStorage pathForTarget:target
        ...
        // Check the QoS, if it's high priority, notify the target that it has a
        // high priority event.
        ...
        dispatch_async(queue, ^ {
            ...
            for (NSString *eventPath in paths) {
                ...
                GDTCOREvent *event = (GDTCOREvent *)GDTCORDecodeArchive(
                    [GDTCOREvent class], eventPath, nil, &error);
                if (event == nil || error) {
                    ...
                    NSString *batchPath = [GDTCORFlatFileStorage
                                           batchPathForTarget:eventSelector.selectedTarget
                                           batchID:batchID
                    ...
                    NSString *destinationPath =
                        [batchPath stringByAppendingPathComponent:fileName];
                    error = nil;
                    ...
                    if (error) {
                        GDTCORLogDebug(
                            @"An event file wasn't moveable into the batch directory: %@",
                            error);
                    }
...
- (void)batchIDsForTarget:(GDTCORTarget)target
    onComplete:
    (nonnull void (^)(NSSet<NSNumber *> *_Nullable))onComplete {
    dispatch_async(_storageQueue, ^ {
        NSFileManager *fileManager = [NSFileManager defaultManager];
        NSError *error;
        NSArray<NSString *> *batchPaths = [fileManager
                                           contentsOfDirectoryAtPath:[GDTCORFlatFileStorage batchDataStoragePath]
                                           error:&error];
        ...
        for (NSString *path in batchPaths) {
            NSDictionary<NSString *, id> *components =
                [self batchComponentsFromFilename:path];
...
- (void)libraryDataForKey:(nonnull NSString *)key
    onFetchComplete:(nonnull void (^)(NSData *_Nullable,
    NSError *_Nullable))onFetchComplete
    setNewValue:(NSData *_Nullable (^_Nullable)(void))setValueBlock {
    dispatch_async(_storageQueue, ^ {
        NSString *dataPath = [[[self class] libraryDataStoragePath]
        ...
        if (setValueBlock) {
            ...
            // The -isKindOfClass check is necessary because without an explicit
            // 'return nil' in the block the implicit return value will be the block
            // itself. The compiler doesn't detect this.
            if (newValue != nil && [newValue isKindOfClass:[NSData class]] &&
                    newValue.length) {
                NSError *newValueError;
                [newValue writeToFile:dataPath
                          options:NSDataWritingAtomic
                          error:&newValueError];
                if (newValueError) {
                    GDTCORLogDebug(@"Error writing new value in libraryDataForKey: %@",
                                   newValueError);
                }
            }
        }
...
- (void)storeLibraryData:(NSData *)data
    ...
    onComplete:
    (nullable void (^)(NSError *_Nullable error))onComplete {
    if (!data || data.length <= 0) {
        if (onComplete) {
            onComplete([NSError errorWithDomain:NSInternalInconsistencyException
                                code:-1
                                userInfo:nil]);
        }
        return;
...
- (void)removeLibraryDataForKey:(nonnull NSString *)key
    onComplete:(nonnull void (^)(NSError *_Nullable error))
    onComplete {
    dispatch_async(_storageQueue, ^ {
        NSError *error;
        NSString *dataPath = [[[self class] libraryDataStoragePath]
...
- (void)hasEventsForTarget:(GDTCORTarget)target
    onComplete:(void (^)(BOOL hasEvents))onComplete {
    dispatch_async(_storageQueue, ^ {
        NSFileManager *fileManager = [NSFileManager defaultManager];
        NSString *targetPath =
        [NSString stringWithFormat:@"%@/%ld",
                  [GDTCORFlatFileStorage eventDataStoragePath],
                  (long)target];
        ...
        [fileManager enumeratorAtPath:targetPath];
...
- (void)checkForExpirations {
    dispatch_async(_storageQueue, ^ {
        ...
        // TODO: Storage may not have enough context to remove batches because a
        // batch may be being uploaded but the storage has not context of it.

        // Find expired batches and move their events back to the main storage.
        // If a batch contains expired events they are expected to be removed
        // further in the method together with other expired events in the main
        // storage.
        NSString *batchDataPath = [GDTCORFlatFileStorage batchDataStoragePath];
        NSArray<NSString *> *batchDataPaths =
        [fileManager contentsOfDirectoryAtPath:batchDataPath error:nil];
        for (NSString *path in batchDataPaths) {
            NSString *fileName = [path lastPathComponent];
            NSDictionary<NSString *, id> *batchComponents =
            [self batchComponentsFromFilename:fileName];
            NSDate *expirationDate =
            batchComponents[kGDTCORBatchComponentsExpirationKey];
        ...
        NSDirectoryEnumerator *enumerator =
            [fileManager enumeratorAtPath:eventDataPath];
        NSString *path;
        while ((path = [enumerator nextObject])) {
            NSString *fileName = [path lastPathComponent];
            NSDictionary<NSString *, id> *eventComponents =
                [self eventComponentsFromFilename:fileName];
            NSDate *expirationDate =
                eventComponents[kGDTCOREventComponentsExpirationKey];
            if (expirationDate != nil && expirationDate.timeIntervalSince1970 < now) {
                NSString *pathToDelete =
                    [eventDataPath stringByAppendingPathComponent:path];
                NSError *error;
                [fileManager removeItemAtPath:pathToDelete error:&error];
                if (error != nil) {
                    GDTCORLogDebug(@"There was an error deleting an expired item: %@",
                                   error);
                } else {
...
- (void)storageSizeWithCallback:(void (^)(uint64_t storageSize))onComplete {
    dispatch_async(_storageQueue, ^ {
        ...
        enumerator =
            [[NSFileManager defaultManager] enumeratorAtPath:libraryDataPath];
        while ([enumerator nextObject]) {
            NSFileAttributeType fileType = enumerator.fileAttributes[NSFileType];
            if ([fileType isEqual:NSFileTypeRegular]) {
                NSNumber *fileSize = enumerator.fileAttributes[NSFileSize];
        ...
        enumerator =
            [[NSFileManager defaultManager] enumeratorAtPath:batchDataPath];
        while ([enumerator nextObject]) {
            NSFileAttributeType fileType = enumerator.fileAttributes[NSFileType];
            if ([fileType isEqual:NSFileTypeRegular]) {
                NSNumber *fileSize = enumerator.fileAttributes[NSFileSize];
...
/** Looks for directory paths containing events for a batch with the specified
 * ID.
 * @param batchID A batch ID.
 * @param outError A pointer to `NSError *` to assign as possible error to.
 * @return An array of an array of paths to directories for event batches with a
 * specified batch ID or `nil` in the case of an error. Usually returns a single
 * path but potentially return more in cases when the app is terminated while
 * uploading a batch.
 */
- (nullable NSArray<NSString *> *)batchDirPathsForBatchID:(NSNumber *)batchID
    ...
    NSFileManager *fileManager = [NSFileManager defaultManager];
    NSError *error;
    NSArray<NSString *> *batches = [fileManager
                                    contentsOfDirectoryAtPath:[GDTCORFlatFileStorage batchDataStoragePath]
                                    error:&error];
    if (batches == nil) {
        *outError = error;
        GDTCORLogDebug(
            @"Failed to find event file paths for batchID: %@, error: %@", batchID,
            error);
        return nil;
    }
    ...
    for (NSString *path in batches) {
        ...
        if ([pathBatchID isEqual:batchID]) {
            NSString *batchDirPath = [[GDTCORFlatFileStorage batchDataStoragePath]
...
/** Makes a copy of the contents of a directory to a directory at the specified
 * path.*/
- (BOOL)moveContentsOfDirectoryAtPath:(NSString *)sourcePath
    ...
    NSError *error;
    NSArray<NSString *> *contentsPaths =
        [fileManager contentsOfDirectoryAtPath:sourcePath error:&error];
    ...
    for (NSString *path in contentsPaths) {
        NSString *contentDestinationPath =
            [destinationPath stringByAppendingPathComponent:path];
        NSString *contentSourcePath =
            [sourcePath stringByAppendingPathComponent:path];
    ...
    if (errors.count == 0) {
        ...
        NSError *combinedError =
            [NSError errorWithDomain:@"GDTCORFlatFileStorage"
                     code:-1
...
- (void)syncThreadUnsafeRemoveBatchWithID:(nonnull NSNumber *)batchID
    deleteEvents:(BOOL)deleteEvents {
    NSError *error;
    NSArray<NSString *> *batchDirPaths = [self batchDirPathsForBatchID:batchID
                                               error:&error];
    ...
    for (NSString *batchDirPath in batchDirPaths) {
        if (deleteEvents) {
            ...
            NSDictionary<NSString *, id> *components =
                [self batchComponentsFromFilename:batchDirName];
            ...
            // `- [NSFileManager moveItemAtPath:toPath:error:]` method fails if an
            // item by the destination path already exists (which usually is the case
            // for the current method). Move the events one by one instead.
            if ([self moveContentsOfDirectoryAtPath:batchDirPath
                         to:destinationPath
                         error:&error]) {
                GDTCORLogDebug(
                    @"Batched events at path: %@ moved back to the storage: %@",
                    batchDirPath, destinationPath);
            } else {
                GDTCORLogDebug(@"Error encountered whilst moving events back: %@",
                               error);
            }

            // Even if not all events where moved back to the storage, there is not
            // much can be done at this point, so cleanup batch directory now to avoid
            // clattering.
            removeBatchDir(batchDirPath);
        }
    }
...
+ (NSString *)eventDataStoragePath {
    ...
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^ {
        eventDataPath = [NSString stringWithFormat:@"%@/%@/gdt_event_data",
                                  GDTCORRootDirectory().path,
    ...
    [[NSFileManager defaultManager] createDirectoryAtPath:eventDataPath
                                    ...
                                    attributes:0
                                    error:&error];
    GDTCORAssert(error == nil, @"Creating the library data path failed: %@",
                 error);
...
+ (NSString *)batchDataStoragePath {
    ...
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^ {
        batchDataPath = [NSString stringWithFormat:@"%@/%@/gdt_batch_data",
                                  GDTCORRootDirectory().path,
                                  NSStringFromClass([self class])];
    });
    NSError *error;
...
+ (NSString *)libraryDataStoragePath {
    ...
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^ {
        libraryDataPath = [NSString
                           stringWithFormat:@"%@/%@/gdt_library_data", GDTCORRootDirectory().path,
                           NSStringFromClass([self class])];
    });
    NSError *error;
...
+ (NSString *)batchPathForTarget:(GDTCORTarget)target
    ...
    return [NSString
            stringWithFormat:@"%@/%ld%@%@%@%llu",
            [GDTCORFlatFileStorage batchDataStoragePath],
            (long)target, kMetadataSeparator, batchID,
            kMetadataSeparator,
...
+ (NSString *)pathForTarget:(GDTCORTarget)target
    ...
    return [NSString
            stringWithFormat:@"%@/%ld/%@%@%@%@%llu%@%@",
            [GDTCORFlatFileStorage eventDataStoragePath],
            (long)target, eventID, kMetadataSeparator, qosTier,
            kMetadataSeparator,
            ((uint64_t)expirationDate.timeIntervalSince1970),
...
void (^completion)(NSSet<NSString *> *) = onComplete == nil ? ^(NSSet<NSString *> *paths) {} :
    onComplete;
    dispatch_async(_storageQueue, ^ {
        ...
        [fileManager contentsOfDirectoryAtPath:targetPath error:&error];
        if (error) {
            GDTCORLogDebug(
                @"There was an error reading the contents of the target path: %@",
                error);
            completion(paths);
            return;
        }
        ...
        BOOL checkingAnything =
        checkingIDs == NO && checkingQosTiers == NO && checkingMappingIDs == NO;
        for (NSString *path in dirPaths) {
            ...
            if (!eventComponents) {
                GDTCORLogDebug(
                    @"There was an error reading the filename components: %@",
                    eventComponents);
                continue;
            }
            ...
            NSNumber *eventIDMatch =
                checkingIDs ? @([eventIDs containsObject:eventID]) : nil;
            NSNumber *qosTierMatch =
                checkingQosTiers ? @([qosTiers containsObject:qosTier]) : nil;
            NSNumber *mappingIDMatch =
                checkingMappingIDs
                ? @([mappingIDs
                     containsObject:[mappingID stringByRemovingPercentEncoding]])
                : nil;
...
- (void)nextBatchID:(void (^)(NSNumber *_Nullable batchID))nextBatchID {
    ...
    [self libraryDataForKey:gBatchIDCounterKey
          onFetchComplete:^(NSData *_Nullable data,
         NSError *_Nullable getValueError) {
             if (!getValueError) {
    ...
    setNewValue:^NSData *_Nullable(void) {
        if (lastBatchID != -1) {
            ...
            return [NSData dataWithBytes:&incrementedValue
...
- (nullable NSDictionary<NSString *, id> *)eventComponentsFromFilename:
    ...
    NSArray<NSString *> *components =
        [fileName componentsSeparatedByString:kMetadataSeparator];
    if (components.count >= 4) {
        NSString *eventID = components[0];
        NSNumber *qosTier = @(components[1].integerValue);
        NSDate *expirationDate =
            [NSDate dateWithTimeIntervalSince1970:components[2].longLongValue];
        NSString *mappingID =
            [[components subarrayWithRange:NSMakeRange(3, components.count - 3)]
             componentsJoinedByString:kMetadataSeparator];
        if (eventID == nil || qosTier == nil || mappingID == nil ||
                expirationDate == nil) {
            GDTCORLogDebug(
                @"There was an error parsing the event filename components: %@",
                components);
            return nil;
        }
        return @ {
kGDTCOREventComponentsEventIDKey :
            eventID,
kGDTCOREventComponentsQoSTierKey :
            qosTier,
kGDTCOREventComponentsExpirationKey :
            expirationDate,
kGDTCOREventComponentsMappingIDKey :
            mappingID
        };
    }
...
- (nullable NSDictionary<NSString *, id> *)batchComponentsFromFilename:
    ...
    if (components.count == 3) {
        ...
        NSDate *expirationDate =
            [NSDate dateWithTimeIntervalSince1970:components[2].doubleValue];
        if (target == nil || batchID == nil || expirationDate == nil) {
            GDTCORLogDebug(
                @"There was an error parsing the batch filename components: %@",
                components);
            return nil;
        }
        return @ {
kGDTCORBatchComponentsTargetKey :
            target,
kGDTCORBatchComponentsBatchIDKey :
            batchID,
kGDTCORBatchComponentsExpirationKey :
            expirationDate
        };
    }
...
- (void)appWillBackground:(GDTCORApplication *)app {
    dispatch_async(_storageQueue, ^ {
        // Immediately request a background task to run until the end of the current
        // queue of work, and cancel it once the work is done.
