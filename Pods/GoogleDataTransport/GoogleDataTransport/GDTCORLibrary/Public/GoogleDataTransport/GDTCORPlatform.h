...
#if !TARGET_OS_WATCH
/** Define SCNetworkReachabilityFlags as GDTCORNetworkReachabilityFlags on
 * non-watchOS. */
typedef SCNetworkReachabilityFlags GDTCORNetworkReachabilityFlags;

/** Define SCNetworkReachabilityRef as GDTCORNetworkReachabilityRef on
 * non-watchOS. */
...
/** Returns a URL to the root directory under which all GDT-associated data must
 * be saved.
 *
 * @return A URL to the root directory under which all GDT-associated data must
 * be saved.
 */
NSURL *GDTCORRootDirectory(void);

/** Compares flags with the reachable flag (on non-watchos with both reachable
 * and connectionRequired flags), if available, and returns YES if network
 * reachable.
 *
...
/** Compares flags with the WWAN reachability flag, if available, and returns
 * YES if present.
 *
 * @param flags The set of reachability flags.
...
/** Generates an enum message GDTCORNetworkType representing network connection
 * type.
 *
...
/** Generates an enum message GDTCORNetworkMobileSubtype representing network
 * connection mobile subtype.
 *
 * @return A GDTCORNetworkMobileSubtype representing network connection mobile
 * subtype.
 */
GDTCORNetworkMobileSubtype GDTCORNetworkMobileSubTypeMessage(void);

/** Identifies the model of the device on which the library is currently working
 * on.
 *
...
/** Writes the given object to the given fileURL and populates the given error
 * if it fails.
 *
 * @param obj The object to encode.
 * @param filePath The path to write the object to. Can be nil if you just need
 * the data.
...
/** Decodes an object of the given class from the given archive path or data and
 * populates the given error if it fails.
 *
 * @param archiveClass The class of the archive's root object.
 * @param archivePath The path to the archived data. Don't use with the
 * archiveData param.
...
/** Creates a background task with the returned identifier if on a suitable
 * platform.
 *
 * @name name The name of the task, useful for debugging which background tasks
 * are running.
 * @param handler The handler block that is called if the background task
 * expires.
 * @return An identifier for the background task, or
