...
typedef void (^GDTCORStorageBatchBlock)(
    NSNumber *_Nullable newBatchID,
...
/** Stores an event and calls onComplete with a non-nil error if anything went
 * wrong.
 *
 * @param event The event to store
 * @param completion The completion block to call after an attempt to store the
 * event has been made.
 */
...
/** Returns YES if some events have been stored for the given target, NO
 * otherwise.
 *
 * @param onComplete The completion block to invoke when determining if there
 * are events is done.
 */
- (void)hasEventsForTarget:(GDTCORTarget)target
    onComplete:(void (^)(BOOL hasEvents))onComplete;

/** Constructs an event batch with the given event selector. Events in this
 * batch will not be returned in any queries or other batches until the batch is
 * removed.
 *
 * @param eventSelector The event selector used to find the events.
 * @param expiration The expiration time of the batch. If
 * removeBatchWithID:deleteEvents:onComplete: is not called within this time
 * frame, the batch will be removed with its events deleted.
 * @param onComplete The completion handler to be called when the events have
 * been fetched.
 */
- (void)batchWithEventSelector:
 ...
 * @param onComplete The completion handler to call when the batch removal
...
- (void)batchIDsForTarget:(GDTCORTarget)target
    onComplete:
    (void (^)(NSSet<NSNumber *> *_Nullable batchIDs))onComplete;

/** Checks the storage for expired events and batches, deletes them if they're
 * expired. */
 ...
 * @param onComplete The callback that will be invoked when removing the data is
 * complete.
 */
 ...
 *
 * @param onComplete The callback that will be invoked once storage size
 ...
 *
 * @param target The target.
 * * @return The storage instance registered for the target, or nil if there is
 * none.
 */
FOUNDATION_EXPORT
id<GDTCORStorageProtocol> _Nullable GDTCORStorageInstanceForTarget(
    GDTCORTarget target);
