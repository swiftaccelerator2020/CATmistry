...
/** A block type that could be run instead of normal assertion logging. No
 * return type, no params.
 */
typedef void (^GDTCORAssertionBlock)(void);

/** Returns the result of executing a soft-linked method present in unit tests
 * that allows a block to be run instead of normal assertion logging. This helps
 * ameliorate issues with catching exceptions that occur on a dispatch_queue.
 *
  ...
  do {                                                                         \
    ...
    if (__builtin_expect(!(condition), 0)) {                                   \
      ...
      if (assertionBlock) {                                                    \
        ...
        __assert_file__ =                                                      \
            __assert_file__ ? __assert_file__ : @"<Unknown File>";             \
...
/** Asserts by logging to the console and throwing an exception if
 * NS_BLOCK_ASSERTIONS is not defined.
 *
 * @param condition The condition you'd expect to be YES.
 */
  ...
  do {                                                                         \
    ...
    if (__builtin_expect(!(condition), 0)) {                                   \
      ...
      if (assertionBlock) {                                                    \
        ...
        __assert_file__ =                                                      \
            __assert_file__ ? __assert_file__ : @"<Unknown File>";             \
        GDTCORLogAssert(YES, __assert_file__, __LINE__, format,                \
                        ##__VA_ARGS__);                                        \
