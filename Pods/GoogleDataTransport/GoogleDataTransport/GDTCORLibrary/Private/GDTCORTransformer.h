...
/** Manages the transforming of events. It's desirable for this to be its own
 * class because running all events through a single instance ensures that
 * transformers are thread-safe. Having a per-transport queue to run on isn't
 * sufficient because transformer objects could maintain state (or at least,
 * there's nothing to stop them from doing that) and the same instances may be
 ...
 * the given event.
 *
 * @note If the app is suspended, a background task will be created to complete
 * work in-progress, but this method will not send any further events until the
 * app is resumed.
 *
 * @param event The event to apply transformers on.
 * @param transformers The list of transformers to apply.
 * @param completion A block to run when an event was written to disk or
 * dropped.
 */
