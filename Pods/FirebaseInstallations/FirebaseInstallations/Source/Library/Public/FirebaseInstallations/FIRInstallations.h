 ...
 * @throw Throws an exception if the default app is not configured yet or
 * required  `FirebaseApp` options are missing.
 */
 ...
 * @returns An instance of `Installations` corresponding to the passed
 * application.
...
/**
 * Retrieves (locally if it exists or from the server) a valid authorization
 * token. An existing token may be invalidated or expired, so it is recommended
 * to fetch the auth token before each server request. The method does the same
 * as `Installations.authTokenForcingRefresh(:, completion:)` with forcing
 * refresh `NO`.
 * @param completion A completion handler which is invoked when the operation
 ...
 * valid authorization token. An existing token may be invalidated or expire, so
 * it is recommended to fetch the auth token before each server request. This
 * method should be used with `forceRefresh == YES` when e.g. a request with the
 * previously fetched auth token failed with "Not Authorized" error.
 * @param forceRefresh If `YES` then the locally cached auth token will be
 * ignored and a new one will be requested from the server. If `NO`, then the
 * locally cached auth token will be returned if exists and has not expired yet.
 * @param completion  A completion handler which is invoked when the operation
 * completes. See `InstallationsTokenHandler` for additional details.
 */
