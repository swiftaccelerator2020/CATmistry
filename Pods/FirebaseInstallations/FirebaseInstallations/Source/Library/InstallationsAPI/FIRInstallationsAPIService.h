...
/**
 * The class is responsible for interacting with HTTP REST API for
 * Installations.
 */
 ...
 * @param installation The `FIRInstallationsItem` instance with the FID to
 * register.
 * @return A promise that is resolved with a new `FIRInstallationsItem` instance
 * with valid tokens. It is rejected with an error in case of a failure.
 */
...
/**
 * Sends a request to delete the installation, related auth tokens and all
 * related data from the server.
 * @param installation The installation to delete.
 * @return Returns a promise that is resolved with the passed installation on
 * successful deletion or is rejected with an error otherwise.
 */
