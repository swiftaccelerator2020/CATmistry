...
- (nullable FIRInstallationsItem *)
  ...
  NSDictionary *responseJSON =
      [FIRInstallationsItem dictionaryFromJSONData:data error:outError];
  if (!responseJSON) {
    return nil;
  }

  NSString *refreshToken =
      [FIRInstallationsItem validStringOrNilForKey:@"refreshToken"
                                          fromDict:responseJSON];
  if (refreshToken == nil) {
    FIRInstallationsItemSetErrorToPointer(
        [FIRInstallationsErrorUtil
            FIDRegistrationErrorWithResponseMissingField:@"refreshToken"],
        outError);
    return nil;
  }
  ...
  if (![authTokenDict isKindOfClass:[NSDictionary class]]) {
    FIRInstallationsItemSetErrorToPointer(
        [FIRInstallationsErrorUtil
            FIDRegistrationErrorWithResponseMissingField:@"authToken"],
        outError);
    return nil;
  }

  FIRInstallationsStoredAuthToken *authToken =
      [FIRInstallationsItem authTokenWithJSONDict:authTokenDict
                                             date:date
                                            error:outError];
  if (authToken == nil) {
    return nil;
  ...
  NSString *installationID =
      [FIRInstallationsItem validStringOrNilForKey:@"fid"
                                          fromDict:responseJSON];
  installation.firebaseInstallationID =
      installationID ?: self.firebaseInstallationID;
...
+ (nullable FIRInstallationsStoredAuthToken *)
    authTokenWithGenerateTokenAPIJSONData:(NSData *)data
                                     date:(NSDate *)date
...
+ (nullable FIRInstallationsStoredAuthToken *)
    authTokenWithJSONDict:(NSDictionary<NSString *, id> *)dict
                     date:(NSDate *)date
                    error:(NSError **)outError {
  ...
  if (token == nil) {
    FIRInstallationsItemSetErrorToPointer(
        [FIRInstallationsErrorUtil
            FIDRegistrationErrorWithResponseMissingField:@"authToken.token"],
        outError);
    return nil;
  }

  NSString *expiresInString = [self validStringOrNilForKey:@"expiresIn"
                                                  fromDict:dict];
  if (expiresInString == nil) {
    FIRInstallationsItemSetErrorToPointer(
        [FIRInstallationsErrorUtil FIDRegistrationErrorWithResponseMissingField:
                                       @"authToken.expiresIn"],
        outError);
    return nil;
  }
  ...
  NSString *expiresInSeconds =
      [expiresInString substringToIndex:expiresInString.length - 1];
...
+ (nullable NSDictionary<NSString *, id> *)dictionaryFromJSONData:(NSData *)data
  ...
  if (![responseJSON isKindOfClass:[NSDictionary class]]) {
    FIRInstallationsItemSetErrorToPointer(
        [FIRInstallationsErrorUtil JSONSerializationError:error], outError);
    return nil;
  }
...
+ (NSString *)validStringOrNilForKey:(NSString *)key
