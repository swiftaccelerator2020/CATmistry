...
NSTimeInterval const kFIRInstallationsTokenExpirationThreshold =
    60 * 60; // 1 hour.

static NSString *const kKeychainService =
...
@property(nonatomic, readonly) id<FIRInstallationsBackoffControllerProtocol>
backoffController;

@property(nonatomic, readonly)
FIRInstallationsSingleOperationPromiseCache<FIRInstallationsItem *>
...
@property(nonatomic, readonly)
FIRInstallationsSingleOperationPromiseCache<NSNull *>
*deleteInstallationPromiseCache;
@end
...
- (instancetype)initWithGoogleAppID:(NSString *)appID
    ...
    NSString *serviceName =
        [FIRInstallationsIDController keychainServiceWithAppID:appID];
    GULKeychainStorage *secureStorage =
        [[GULKeychainStorage alloc] initWithService:serviceName];
    ...
    NSString *APIServiceProjectID =
        (projectID.length > 0) ? projectID : GCMSenderID;
    FIRInstallationsAPIService *apiService =
        [[FIRInstallationsAPIService alloc] initWithAPIKey:APIKey
                                            projectID:APIServiceProjectID];
...
/// `installationsStore`.
- (instancetype)
    initWithGoogleAppID:(NSString *)appID
    appName:(NSString *)appName
    ...
    if (self) {
        ...
        _authTokenPromiseCache =
            [[FIRInstallationsSingleOperationPromiseCache alloc]
        initWithNewOperationHandler:^FBLPromise *_Nonnull {
            FIRInstallationsIDController *strongSelf = weakSelf;
            return
            [strongSelf installationWithValidAuthTokenForcingRefresh:NO];
        }];

        _authTokenForcingRefreshPromiseCache =
            [[FIRInstallationsSingleOperationPromiseCache alloc]
        initWithNewOperationHandler:^FBLPromise *_Nonnull {
            FIRInstallationsIDController *strongSelf = weakSelf;
            return
            [strongSelf installationWithValidAuthTokenForcingRefresh:YES];
        }];

        _deleteInstallationPromiseCache =
            [[FIRInstallationsSingleOperationPromiseCache alloc]
        initWithNewOperationHandler:^FBLPromise *_Nonnull {
            FIRInstallationsIDController *strongSelf = weakSelf;
...
- (FBLPromise<FIRInstallationsItem *> *)createGetInstallationItemPromise {
    FIRLogDebug(kFIRLoggerInstallations,
                kFIRInstallationsMessageCodeNewGetInstallationOperationCreated,
                @"%s, appName: %@", __PRETTY_FUNCTION__, self.appName);
    ...
    // Initiate registration process on success if needed, but return the
    // installation without waiting for it.
...
- (FBLPromise<FIRInstallationsItem *> *)getStoredInstallation {
    return [self.installationsStore installationForAppID:self.appID
                                    appName:self.appName]
    .validate(^BOOL(FIRInstallationsItem *installation) {
        BOOL isValid = NO;
...
- (FBLPromise<FIRInstallationsItem *> *)saveInstallation:
    (FIRInstallationsItem *)installation {
...
/**
 * Tries to migrate IID data stored by FirebaseInstanceID SDK or generates a new
 * Installation ID if not found.
 */
- (FBLPromise<FIRInstallationsItem *> *)migrateOrGenerateInstallation {
    if (![self isDefaultApp]) {
        ...
        FIRInstallationsItem *installation =
            [self createInstallationWithFID:[FIRInstallationsItem generateFID]
                  IIDDefaultToken:nil];
        return [FBLPromise resolvedWith:installation];
    }

    return [[[FBLPromise all:@[
                  [self.IIDStore existingIID], [self.IIDTokenStore existingIIDDefaultToken]
    ]] then:^id _Nullable(NSArray *_Nullable results) {
        NSString *existingIID = results[0];
        NSString *IIDDefaultToken = results[1];

        return [self createInstallationWithFID:existingIID
                     IIDDefaultToken:IIDDefaultToken];
    }] recover:^id _Nullable(NSError *_Nonnull error) {
        return [self createInstallationWithFID:[FIRInstallationsItem generateFID]
                     IIDDefaultToken:nil];
    }];
}
...
- (FIRInstallationsItem *)createInstallationWithFID:(NSString *)FID
    IIDDefaultToken:
    (nullable NSString *)IIDDefaultToken {
    FIRInstallationsItem *installation =
...
- (FBLPromise<FIRInstallationsItem *> *)registerInstallationIfNeeded:
    ...
    if (![self.backoffController isNextRequestAllowed]) {
        return [FIRInstallationsErrorUtil
                rejectedPromiseWithError:[FIRInstallationsErrorUtil
                                          backoffIntervalWaitError]];
    }
    ...
    .catch(^(NSError *_Nonnull error) {
        ...
        if ([self doesRegistrationErrorRequireConfigChange:error]) {
            FIRLogError(
                ...
                @"Firebase Installation registration failed for app with name: "
                @"%@, error:\n"
    ...
    .then(^FIRInstallationsItem *(
    FIRInstallationsItem *registeredInstallation) {
        // Server may respond with a different FID if the sent one cannot be
        // accepted.
...
- (FBLPromise<FIRInstallationsItem *> *)getAuthTokenForcingRefresh:
    (BOOL)forceRefresh {
    if (forceRefresh ||
            [self.authTokenForcingRefreshPromiseCache getExistingPendingPromise] !=
            nil) {
        return [self.authTokenForcingRefreshPromiseCache
                getExistingPendingOrCreateNewPromise];
    } else {
...
- (FBLPromise<FIRInstallationsItem *> *)
    installationWithValidAuthTokenForcingRefresh:(BOOL)forceRefresh {
    FIRLogDebug(kFIRLoggerInstallations,
                kFIRInstallationsMessageCodeNewGetAuthTokenOperationCreated,
                @"-[FIRInstallationsIDController "
                @"installationWithValidAuthTokenForcingRefresh:%@], "
                @"appName: %@",
    ...
    .then(^id(FIRInstallationsItem *registeredInstallation) {
        BOOL isTokenExpiredOrExpiresSoon =
            [registeredInstallation.authToken.expirationDate
...
- (FBLPromise<FIRInstallationsItem *> *)refreshAuthTokenForInstallation:
    ...
    return [[[self.APIService refreshAuthTokenForInstallation:installation]
             then:^id _Nullable(
    FIRInstallationsItem *_Nullable refreshedInstallation) {
        [self updateBackoffWithSuccess:YES APIError:nil];
...
- (FBLPromise<NSNull *> *)deleteInstallation {
    return [self.deleteInstallationPromiseCache
            getExistingPendingOrCreateNewPromise];
}

- (FBLPromise<NSNull *> *)createDeleteInstallationPromise {
    FIRLogDebug(kFIRLoggerInstallations,
                kFIRInstallationsMessageCodeNewDeleteInstallationOperationCreated,
                @"%s, appName: %@", __PRETTY_FUNCTION__, self.appName);

    // Check for ongoing requests first, if there is no a request, then check
    // local storage for existing installation.
...
- (FBLPromise<NSNull *> *)deleteInstallationLocally:
    (FIRInstallationsItem *)installation {
    return [self.installationsStore
            removeInstallationForAppID:installation.appID
...
- (FBLPromise<FIRInstallationsItem *> *)sendDeleteInstallationRequestIfNeeded:
    ...
    case FIRInstallationStatusUnknown:
    case FIRInstallationStatusUnregistered:
        // The installation is not registered, so it is safe to be deleted as is, so
        // return early.
        return [FBLPromise resolvedWith:installation];
        break;
    ...
    return [self.APIService deleteInstallation:installation].recover(
    ^id(NSError *APIError) {
...
- (nullable FBLPromise<FIRInstallationsItem *> *)
    mostRecentInstallationOperation {
    return [self.authTokenForcingRefreshPromiseCache getExistingPendingPromise]
           ...
           ?: [self.getInstallationPromiseCache
               getExistingPendingPromise];
}

#pragma mark - Backoff

- (void)updateBackoffWithSuccess:(BOOL)success
    APIError:(nullable NSError *)APIError {
    if (success) {
        ...
        FIRInstallationsHTTPError *HTTPResponseError =
            (FIRInstallationsHTTPError *)APIError;
        ...
        if (statusCode == FIRInstallationsAuthTokenHTTPCodeInvalidAuthentication ||
            ...
            // These errors are explicitly excluded because they are handled by FIS
            // SDK itself so don't require backoff.
        } else if (statusCode == 400 ||
                   statusCode == 403) { // Explicitly unrecoverable errors.
            [self.backoffController
             registerEvent:FIRInstallationsBackoffEventUnrecoverableFailure];
        } else if (statusCode == 429 ||
                   (statusCode >= 500 &&
                    statusCode < 600)) { // Explicitly recoverable errors.
            [self.backoffController
             registerEvent:FIRInstallationsBackoffEventRecoverableFailure];
    ...
    // If the error class is not `FIRInstallationsHTTPError` it indicates a
    // connection error. Such errors should not change backoff interval.
}
...
+ (NSString *)keychainServiceWithAppID:(NSString *)appID {
    ...
    // Applications on macOS may request access to Keychain items stored by other
    // applications. It means that when the app looks up for a relevant Keychain
    // item in the service scope it will request user password to grant access to
    // the Keychain if there are other Keychain items from other applications
    // stored under the same Keychain Service.
    return [kKeychainService stringByAppendingFormat:@".%@", appID];
#else
    // Use a constant Keychain service for non-macOS because:
    // 1. Keychain items cannot be shared between apps until configured
    // specifically so the service name collisions are not a concern
