...
typedef void (*GULOpenURLContextsIMP)(id, SEL, UIScene *,
...
static GULLoggerService kGULLoggerSwizzler =
    @"[GoogleUtilities/SceneDelegateSwizzler]";
...
#if UISCENE_SUPPORTED
+ (GULSceneDelegateInterceptorID)registerSceneDelegateInterceptor:
    ...
    NSAssert([interceptor conformsToProtocol:@protocol(UISceneDelegate)],
             @"SceneDelegateProxy interceptor does not conform to "
    ...
    GULSceneZeroingWeakContainer *weakObject =
        [[GULSceneZeroingWeakContainer alloc] init];
...
+ (void)unregisterSceneDelegateInterceptorWithID:
    ...
    NSAssert(interceptorID,
             @"SceneDelegateProxy cannot unregister nil interceptor ID.");
    ...
    if (!weakContainer.object) {
        GULLogError(
            ...
            @"SceneDelegateProxy cannot unregister interceptor that was not "
            @"registered. "
            "Interceptor ID %@",
            interceptorID);
        return;
...
+ (void)proxyDestinationSelector:(SEL)destinationSelector
    ...
    IMP sourceImplementation = [GULSceneDelegateSwizzler
                                implementationOfMethodSelector:destinationSelector
                                fromClass:realClass];
...
+ (void)addInstanceMethodWithDestinationSelector:(SEL)destinationSelector
    ...
    if (!class_addMethod(toClass, destinationSelector, methodIMP, types)) {
        GULLogWarning(
            ...
            [NSString stringWithFormat:
                      @"I-SWZ%06ld",
                      (long)kGULSwizzlerMessageCodeSceneDelegateSwizzling009],
 ...
 * selector, executes a GULSceneDelegateInterceptorCallback with the
 * interceptor.
 *
...
+ (void)notifyInterceptorsWithMethodSelector:(SEL)methodSelector
    callback:
    (GULSceneDelegateInterceptorCallback)
    callback API_AVAILABLE(ios(13.0)) {
    if (!callback) {
        return;
    }

    NSDictionary *interceptors =
        [GULSceneDelegateSwizzler interceptors].dictionary;
    [interceptors enumerateKeysAndObjectsUsingBlock:^(id key, id obj,
            ...
            GULLogWarning(
                ...
                @"SceneDelegateProxy cannot find interceptor with ID %@. Removing "
                @"the interceptor.",
                key);
...
- (void)scene:(UIScene *)scene
    ...
    if (@available(iOS 13.0, tvOS 13.0, *)) {
        ...
        NSValue *openURLContextsIMPPointer = [GULSceneDelegateSwizzler
                                              originalImplementationForSelector:methodSelector
                                              object:self];
        GULOpenURLContextsIMP openURLContextsIMP =
            [openURLContextsIMPPointer pointerValue];
...
+ (void)proxySceneDelegateIfNeeded:(UIScene *)scene {
    ...
    // Skip proxying if failed to get the delegate class name for some reason
    // (e.g. `delegate == nil`) or the class has a prefix of
    // kGULAppDelegatePrefix, which means it has been proxied before.
    if (className == nil || [className hasPrefix:kGULSceneDelegatePrefix]) {
        return;
    }

    NSString *classNameWithPrefix =
        [kGULSceneDelegatePrefix stringByAppendingString:className];
    ...
    if (NSClassFromString(newClassName)) {
        GULLogError(
            ...
            @"Cannot create a proxy for Scene Delegate. Subclass already exists. "
            @"Original Class"
            @": %@, subclass: %@",
    ...
    if (class_getInstanceSize(realClass) !=
            class_getInstanceSize(sceneDelegateSubClass)) {
        GULLogError(
            kGULLoggerSwizzler, NO,
            [NSString
             stringWithFormat:
             ...
             (long)
             kGULSwizzlerMessageCodeSceneDelegateSwizzlingInvalidSceneDelegate],
            @"Cannot create subclass of Scene Delegate, because the created "
            @"subclass is not the "
            @"same size. %@",
            className);
    ...
    // Make the newly created class to be the subclass of the real Scene Delegate
    // class.
    ...
    if (object_setClass(scene.delegate, sceneDelegateSubClass)) {
        GULLogDebug(
            ...
            [NSString
             ...
             (long)
             kGULSwizzlerMessageCodeSceneDelegateSwizzlingInvalidSceneDelegate],
            @"Successfully created Scene Delegate Proxy automatically. To disable "
            @"the "
...
+ (NSString *)correctSceneDelegateProxyKey {
    return NSClassFromString(@"FIRCore")
           ? kGULFirebaseSceneDelegateProxyEnabledPlistKey
