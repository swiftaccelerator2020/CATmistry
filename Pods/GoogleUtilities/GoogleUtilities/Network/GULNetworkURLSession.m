...
#pragma clang diagnostic ignored "-Wunguarded-availability"
  /// The session configuration. NSURLSessionConfiguration' is only available on
  /// iOS 7.0 or newer.
...
#pragma clang diagnostic pop
  ...
  /// The path to the directory where all temporary files are stored before
  /// uploading.
...
- (instancetype)initWithNetworkLoggerDelegate:
    (id<GULNetworkLoggerDelegate>)networkLoggerDelegate {
  self = [super init];
  if (self) {
    // Create URL to the directory where all temporary files to upload have to
    // be stored.
    ...
    _sessionID = [NSString
        stringWithFormat:@"%@-%@", kGULNetworkBackgroundSessionConfigIDPrefix,
...
+ (void)handleEventsForBackgroundURLSessionID:(NSString *)sessionID
  ...
  // The session may not be Analytics background. Ignore those that do not have
  // the prefix.
  ...
  if (fetcher != nil) {
    [fetcher addSystemCompletionHandler:systemCompletionHandler
                             forSession:sessionID];
  } else {
    GULLogError(
        kGULLoggerNetwork, NO,
        [NSString stringWithFormat:@"I-NET%06ld",
                                   (long)kGULNetworkMessageCodeNetwork003],
        @"Failed to retrieve background session with ID %@ after app is "
        @"relaunched.",
        sessionID);
  }
}
...
/// Sends an async POST request using NSURLSession for iOS >= 7.0, and returns
/// an ID of the connection.
- (nullable NSString *)
    sessionIDFromAsyncPOSTRequest:(NSURLRequest *)request
  ...
  // Clean up the entire temp folder to avoid temp files that remain in case the
  // previous session crashed and did not clean up.
  ...
  // will allow default network session which runs on the foreground.
  ...
  if (didWriteFile) {
    // Exclude this file from backing up to iTunes. There are conflicting
    // reports that excluding directory from backing up does not exclude files
    // of that directory from backing up.
    ...
    session =
        [NSURLSession sessionWithConfiguration:_sessionConfig
                                      delegate:self
                                 delegateQueue:[NSOperationQueue mainQueue]];
    postRequestTask = [session uploadTaskWithRequest:request
  ...
  if (!session || !postRequestTask) {
    NSError *error = [[NSError alloc]
        initWithDomain:kGULNetworkErrorDomain
              ...
              userInfo:@{
                kGULNetworkErrorContext : @"Cannot create network session"
              }];
  ...
  // Store completion handler because background session does not accept handler
  // block but custom delegate.
...
/// Sends an async GET request using NSURLSession for iOS >= 7.0, and returns an
/// ID of the session.
- (nullable NSString *)
    sessionIDFromAsyncGETRequest:(NSURLRequest *)request
               completionHandler:(GULNetworkURLSessionCompletionHandler)handler
    API_AVAILABLE(ios(7.0)) {
...
/// Called by the NSURLSession when the data task has received some of the
/// expected data. Once the session is completed,
/// URLSession:task:didCompleteWithError will be called and the completion
/// handler will be called with the downloaded data.
...
/// Called by the NSURLSession once the download task is completed. The file is
/// saved in the provided URL so we need to read the data and store into
/// _downloadedData. Once the session is completed,
/// URLSession:task:didCompleteWithError will be called and the completion
/// handler will be called with the downloaded data.
- (void)URLSession:(NSURLSession *)session
  ...
  if (!url.path) {
    [_loggerDelegate GULNetwork_logWithLevel:kGULNetworkLogLevelError
                                 messageCode:kGULNetworkMessageCodeURLSession001
                                     message:@"Unable to read downloaded data "
                                             @"from empty temp path"];
    _downloadedData = nil;
    return;
  }

  NSError *error;
  _downloadedData = [NSData dataWithContentsOfFile:url.path
                                           options:0
                                             error:&error];
...
- (void)URLSession:(NSURLSession *)session
  ...
  if (task.response) {
    // The following assertion should always be true for HTTP requests, see
    // https://goo.gl/gVLxT7.
    ...
    error =
        [[NSError alloc] initWithDomain:kGULNetworkErrorDomain
                                   code:GULErrorCodeNetworkInvalidResponse
                               userInfo:@{
                                 kGULNetworkErrorContext :
  ...
  // This is called without checking the sessionID here since non-background
  // sessions won't have an ID.
  [session finishTasksAndInvalidate];

  // Explicitly remove the session so it won't be reused. The weak map table
  // should remove the session on deallocation, but dealloc may not happen
  // immediately after calling `finishTasksAndInvalidate`.
...
- (void)URLSession:(NSURLSession *)session
  ...
  if ([challenge.protectionSpace.authenticationMethod
        ...
        [self->_loggerDelegate
            GULNetwork_logWithLevel:kGULNetworkLogLevelDebug
                        messageCode:kGULNetworkMessageCodeURLSession007
                            message:@"Cancelling authentication challenge for "
                                    @"host. Host"
    ...
    //
    // The delegate queue may be the main thread. Trust evaluation could cause
    // some blocking network activity, so we must evaluate async, as documented
    // at https://developer.apple.com/library/ios/technotes/tn2232/
...
#pragma clang dianostic pop
      ...
      if (trustError != errSecSuccess) {
        ...
        // Having a trust level "unspecified" by the user is the usual result,
        // described at https://developer.apple.com/library/mac/qa/qa1360
        shouldAllow = (trustEval == kSecTrustResultUnspecified ||
  ...
  if (!handler) {
    [_loggerDelegate GULNetwork_logWithLevel:kGULNetworkLogLevelError
                                 messageCode:kGULNetworkMessageCodeURLSession009
                                     message:@"Cannot store nil system "
                                             @"completion handler in network"];
    return;
  }
  ...
  if (!identifier.length) {
    [_loggerDelegate GULNetwork_logWithLevel:kGULNetworkLogLevelError
                                 messageCode:kGULNetworkMessageCodeURLSession010
                                     message:@"Cannot store system completion "
                                             @"handler with empty network "
                                              "session identifier"];
    return;
  }
  ...
  if (systemCompletionHandlers[identifier]) {
    [_loggerDelegate GULNetwork_logWithLevel:kGULNetworkLogLevelWarning
                                 messageCode:kGULNetworkMessageCodeURLSession011
                                     message:@"Got multiple system handlers "
...
/// Calls the system provided completion handler with the session ID stored in
/// the dictionary. The handler will be removed from the dictionary after being
/// called.
- (void)callSystemCompletionHandler:(NSString *)identifier {
  GULMutableDictionary *systemCompletionHandlers =
      [[self class] sessionIDToSystemCompletionHandlerDictionary];
  GULNetworkSystemCompletionHandler handler =
      [systemCompletionHandlers objectForKey:identifier];
...
/// Creates a background session configuration with the session ID using the
/// supported method.
- (NSURLSessionConfiguration *)backgroundSessionConfigWithSessionID:
    (NSString *)sessionID API_AVAILABLE(ios(7.0)) {
  ...
  // iOS 8/10.10 builds require the new backgroundSessionConfiguration method
  // name.
...
- (void)maybeRemoveTempFilesAtURL:(NSURL *)folderURL
                     expiringTime:(NSTimeInterval)staleTime {
  ...
  if (error && error.code != NSFileReadNoSuchFileError) {
    [_loggerDelegate GULNetwork_logWithLevel:kGULNetworkLogLevelDebug
                                 messageCode:kGULNetworkMessageCodeURLSession012
                                     message:@"Cannot get files from the "
                                             @"temporary network folder. Error"
                                     context:error];
    return;
  }
...
/// Removes the temporary file written to disk for sending the request. It has
/// to be cleaned up after the session is done.
  ...
  if (![fileManager removeItemAtURL:fileURL error:&error] &&
    ...
    [_loggerDelegate GULNetwork_logWithLevel:kGULNetworkLogLevelError
                                 messageCode:kGULNetworkMessageCodeURLSession013
                                     message:@"Failed to remove temporary "
                                             @"uploading data file. Error"
...
+ (instancetype)fetcherWithSessionIdentifier:(NSString *)sessionIdentifier {
  GULNetworkURLSession *session =
      [self sessionFromFetcherMapForSessionID:sessionIdentifier];
  if (!session && [sessionIdentifier
                      hasPrefix:kGULNetworkBackgroundSessionConfigIDPrefix]) {
...
/// Returns a map of the fetcher by session ID. Creates a map if it is not
/// created. When reading and writing from/to the session map, don't use this
/// method directly. To avoid thread safety issues, use one of the helper
/// methods at the bottom of the file: setSessionInFetcherMap:forSessionID:,
/// sessionFromFetcherMapForSessionID:
...
/// Returns a map of system provided completion handler by session ID. Creates a
/// map if it is not created.
...
- (NSURL *)temporaryFilePathWithSessionID:(NSString *)sessionID {
  NSString *tempName =
      [NSString stringWithFormat:@"GULUpload_temp_%@", sessionID];
  return [_networkDirectoryURL URLByAppendingPathComponent:tempName];
}

/// Makes sure that the directory to store temp files exists. If not, tries to
/// create it and returns YES. If there is anything wrong, returns NO.
  ...
  if (error && error.code != NSFileReadNoSuchFileError) {
    [_loggerDelegate GULNetwork_logWithLevel:kGULNetworkLogLevelWarning
                                 messageCode:kGULNetworkMessageCodeURLSession014
                                     message:@"Error while trying to access "
                                             @"Network temp folder. Error"
                                     context:error];
  }
  ...
  if (preventBackupError) {
    [_loggerDelegate GULNetwork_logWithLevel:kGULNetworkLogLevelError
                                 messageCode:kGULNetworkMessageCodeURLSession016
                                     message:@"Cannot exclude temporary folder "
                                             @"from iTunes backup"];
  }
}
...
+ (void)setSessionInFetcherMap:(GULNetworkURLSession *)session
  ...
  if (existingSession) {
    if (session) {
      NSString *message = [NSString
          stringWithFormat:@"Discarding session: %@", existingSession];
      [existingSession->_loggerDelegate
          GULNetwork_logWithLevel:kGULNetworkLogLevelInfo
...
+ (nullable GULNetworkURLSession *)sessionFromFetcherMapForSessionID:
    (NSString *)sessionID {
  [[self sessionIDToFetcherMapReadWriteLock] lock];
  GULNetworkURLSession *session =
      [[[self class] sessionIDToFetcherMap] objectForKey:sessionID];
...
// Always use the request parameters even if the default session configuration
// is more restrictive.
