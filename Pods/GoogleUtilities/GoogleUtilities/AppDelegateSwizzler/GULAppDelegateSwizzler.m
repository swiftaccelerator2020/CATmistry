...
// Implementations need to be typed before calling the implementation directly
// to cast the arguments and the return types correctly. Otherwise, it will
// crash the app.
typedef BOOL (*GULRealOpenURLSourceApplicationAnnotationIMP)(id, SEL,
                                                             GULApplication *,
                                                             NSURL *,
                                                             NSString *, id);

typedef BOOL (*GULRealOpenURLOptionsIMP)(id, SEL, GULApplication *, NSURL *,
                                         NSDictionary<NSString *, id> *);

#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wstrict-prototypes"
typedef void (*GULRealHandleEventsForBackgroundURLSessionIMP)(id, SEL,
                                                              GULApplication *,
                                                              NSString *,
                                                              void (^)());
...
typedef BOOL (*GULRealContinueUserActivityIMP)(
    id, SEL, GULApplication *, NSUserActivity *,
    void (^)(NSArray *restorableObjects));

typedef void (*GULRealDidRegisterForRemoteNotificationsIMP)(id, SEL,
                                                            GULApplication *,
                                                            NSData *);

typedef void (*GULRealDidFailToRegisterForRemoteNotificationsIMP)(
    id, SEL, GULApplication *, NSError *);

typedef void (*GULRealDidReceiveRemoteNotificationIMP)(id, SEL,
                                                       GULApplication *,
                                                       NSDictionary *);

#if !TARGET_OS_WATCH && !TARGET_OS_OSX
typedef void (*GULRealDidReceiveRemoteNotificationWithCompletionIMP)(
    id, SEL, GULApplication *, NSDictionary *,
...
// Since Firebase SDKs also use this for app delegate proxying, in order to not
// be a breaking change we disable App Delegate proxying when either of these
// two flags are set to NO.

/** Plist key that allows Firebase developers to disable App and Scene Delegate
 * Proxying. */
static NSString *const kGULFirebaseAppDelegateProxyEnabledPlistKey =
    @"FirebaseAppDelegateProxyEnabled";

/** Plist key that allows developers not using Firebase to disable App and Scene
 * Delegate Proxying.
 */
 ...
 *
 *  We have to opt out of referencing APNS related App Delegate methods directly
 * to prevent an Apple review warning email about missing Push Notification
 * Entitlement (like here:
 * https://github.com/firebase/firebase-ios-sdk/issues/2807). From our
 * experience, the warning is triggered when any of the symbols is present in
 * the application sent to review, even if the code is never executed. Because
 * GULAppDelegateSwizzler may be used by applications that are not using APNS we
 * have to refer to the methods indirectly using selector constructed from
 *  string.
 *
 *  NOTE: None of the methods is proxied unless it is explicitly requested by
 * calling the method
...
/**
 * This class is necessary to store the delegates in an NSArray without
 * retaining them. [NSValue valueWithNonRetainedObject] also provides this
 * functionality, but does not provide a zeroing pointer. This will cause
 * EXC_BAD_ACCESS when trying to access the object after it is dealloced.
 * Instead, this container stores a weak, zeroing reference to the object, which
...
- (void)observeValueForKeyPath:(NSString *)keyPath
  ...
  if ([keyPath isEqual:kGULAppDelegateKeyPath]) {
    ...
    // Free the stored app delegate instance because it has been changed to a
    // different instance to avoid keeping it alive forever.
...
+ (BOOL)isAppDelegateProxyEnabled {
  ...
  id isFirebaseProxyEnabledPlistValue =
      infoDictionary[kGULFirebaseAppDelegateProxyEnabledPlistKey];
  ...
  if ([isFirebaseProxyEnabledPlistValue isKindOfClass:[NSNumber class]]) {
    isFirebaseAppDelegateProxyEnabled =
        [isFirebaseProxyEnabledPlistValue boolValue];
  }

  if ([isGoogleProxyEnabledPlistValue isKindOfClass:[NSNumber class]]) {
    isGoogleUtilitiesAppDelegateProxyEnabled =
        [isGoogleProxyEnabledPlistValue boolValue];
  }

  // Only deactivate the proxy if it is explicitly disabled by app developers
  // using either one of the plist flags.
  return isFirebaseAppDelegateProxyEnabled &&
         isGoogleUtilitiesAppDelegateProxyEnabled;
}
...
+ (GULAppDelegateInterceptorID)registerAppDelegateInterceptor:
  ...
  NSAssert([interceptor conformsToProtocol:@protocol(GULApplicationDelegate)],
           @"AppDelegateProxy interceptor does not conform to "
  ...
  NSString *interceptorID =
      [NSString stringWithFormat:@"%@%p", kGULAppDelegatePrefix, interceptor];
...
+ (void)unregisterAppDelegateInterceptorWithID:
  ...
  NSAssert(interceptorID,
           @"AppDelegateProxy cannot unregister nil interceptor ID.");
  ...
  GULZeroingWeakContainer *weakContainer =
      [GULAppDelegateSwizzler interceptors][interceptorID];
  if (!weakContainer.object) {
    GULLogError(
        ...
        @"AppDelegateProxy cannot unregister interceptor that was not "
        @"registered. "
         "Interceptor ID %@",
        interceptorID);
    return;
...
+ (void)proxyOriginalDelegateIncludingAPNSMethods {
  ...
  dispatch_once(&sProxyAppDelegateRemoteNotificationOnceToken, ^{
    id<GULApplicationDelegate> appDelegate =
        [GULAppDelegateSwizzler sharedApplication].delegate;

    NSMutableDictionary *realImplementationsBySelector =
        [objc_getAssociatedObject(appDelegate, &kGULRealIMPBySelectorKey)
            mutableCopy];

    [self
        proxyRemoteNotificationsMethodsWithAppDelegateSubClass:
            gAppDelegateSubclass
                                                     realClass:
                                                         gOriginalAppDelegateClass
...
+ (GULApplication *)sharedApplication {
  ...
  if (uiApplicationClass &&
      [uiApplicationClass
          respondsToSelector:(NSSelectorFromString(@"sharedApplication"))]) {
...
/** Creates a new subclass of the class of the given object and sets the isa
 * value of the given object to the new subclass. Additionally this copies
 * methods to that new subclass that allow us to intercept UIApplicationDelegate
 * methods. This is better known as isa swizzling.
 *
 *  @param appDelegate The object to which you want to isa swizzle. This has to
 * conform to the UIApplicationDelegate subclass.
 *  @return Returns the new subclass.
 */
+ (nullable Class)createSubclassWithObject:
    (id<GULApplicationDelegate>)appDelegate {
  Class realClass = [appDelegate class];

  // Create GUL_<RealAppDelegate>_<UUID>
  NSString *classNameWithPrefix = [kGULAppDelegatePrefix
      stringByAppendingString:NSStringFromClass(realClass)];
  NSString *newClassName = [NSString
      stringWithFormat:@"%@-%@", classNameWithPrefix, [NSUUID UUID].UUIDString];
  ...
  if (NSClassFromString(newClassName)) {
    GULLogError(
        ...
        @"Cannot create a proxy for App Delegate. Subclass already exists. "
        @"Original Class: "
        @"%@, subclass: %@",
  ...
  // Register the new class as subclass of the real one. Do not allocate more
  // than the real class size.
  Class appDelegateSubClass =
      objc_allocateClassPair(realClass, newClassName.UTF8String, 0);
  ...
  SEL continueUserActivitySEL = @selector(application:
  ...
  // Add the following methods from GULAppDelegate class, and store the real
  // implementation so it can forward to the real one. For
  ...
  if ([appDelegate respondsToSelector:applicationOpenURLOptionsSEL]) {
    // Only add the application:openURL:options: method if the original
    // AppDelegate implements it. This fixes a bug if an app only implements
    // application:openURL:sourceApplication:annotation: (if we add the
    // `options` method, iOS sees that one exists and does not call the
    // `sourceApplication` method, which in this case is the only one the app
    // implements).
  ...
  SEL handleEventsForBackgroundURLSessionSEL = @selector
  ...
  [GULAppDelegateSwizzler
      addInstanceMethodWithDestinationSelector:@selector(description)
  ...
  // The subclass size has to be exactly the same size with the original class
  // size. The subclass cannot have more ivars/properties than its superclass
  // since it will cause an offset in memory that can lead to overwriting the
  // isa of an object in the next frame.
  if (class_getInstanceSize(realClass) !=
      class_getInstanceSize(appDelegateSubClass)) {
    GULLogError(
        kGULLoggerSwizzler, NO,
        [NSString stringWithFormat:
                      @"I-SWZ%06ld",
                      (long)kGULSwizzlerMessageCodeAppDelegateSwizzling007],
        @"Cannot create subclass of App Delegate, because the created subclass "
        @"is not the "
        @"same size. %@",
        NSStringFromClass(realClass));
  ...
  // Make the newly created class to be the subclass of the real App Delegate
  // class.
  ...
  if (object_setClass(appDelegate, appDelegateSubClass)) {
    GULLogDebug(
        ...
        @"Successfully created App Delegate Proxy automatically. To disable "
        @"the "
...
+ (void)proxyRemoteNotificationsMethodsWithAppDelegateSubClass:
            (Class)appDelegateSubClass
  ...
  [self proxyDestinationSelector:didRegisterForRemoteNotificationsSEL
      implementationsFromSourceSelector:
          didRegisterForRemoteNotificationsDonorSEL
                              fromClass:[GULAppDelegateSwizzler class]
                                toClass:appDelegateSubClass
                              realClass:realClass
       storeDestinationImplementationTo:realImplementationsBySelector];
  ...
  [self proxyDestinationSelector:didFailToRegisterForRemoteNotificationsSEL
      implementationsFromSourceSelector:
          didFailToRegisterForRemoteNotificationsDonorSEL
                              fromClass:[GULAppDelegateSwizzler class]
                                toClass:appDelegateSubClass
                              realClass:realClass
       storeDestinationImplementationTo:realImplementationsBySelector];
  ...
  SEL didReceiveRemoteNotificationSEL =
      NSSelectorFromString(kGULDidReceiveRemoteNotificationSEL);
  ...
  SEL didReceiveRemoteNotificationWithCompletionDonorSEL = @selector
      (application:donor_didReceiveRemoteNotification:fetchCompletionHandler:);
  if ([appDelegate
    ...
    // if the original AppDelegate implements it. This fixes a bug if an app
    // only implements application:didReceiveRemoteNotification: (if we add the
    // method with completion, iOS sees that one exists and does not call the
    // method without the completion, which in this case is the only one the app
    // implements).
...
/// We have to do this to invalidate the cache that caches the original
/// respondsToSelector of openURL handlers. Without this, it won't call the
/// default implementations because the system checks and caches them. Register
/// KVO only once. Otherwise, the observing method will be called as many times
/// as being registered.
...
+ (nullable NSValue *)originalImplementationForSelector:(SEL)selector
                                                 object:(id)object {
...
+ (void)proxyDestinationSelector:(SEL)destinationSelector
    implementationsFromSourceSelector:(SEL)sourceSelector
     ...
     storeDestinationImplementationTo:
         (NSMutableDictionary<NSString *, NSValue *> *)
             destinationImplementationsBySelector {
  ...
  IMP sourceImplementation =
      [GULAppDelegateSwizzler implementationOfMethodSelector:destinationSelector
                                                   fromClass:realClass];
  NSValue *sourceImplementationPointer =
      [NSValue valueWithPointer:sourceImplementation];

  NSString *destinationSelectorString =
      NSStringFromSelector(destinationSelector);
  destinationImplementationsBySelector[destinationSelectorString] =
      sourceImplementationPointer;
}

/** Copies a method identified by the methodSelector from one class to the
 * other. After this method is called, performing [toClassInstance
 * methodSelector] will be similar to calling [fromClassInstance
 * methodSelector]. This method does nothing if toClass already has a method
 *  identified by methodSelector.
 *
 *  @param methodSelector The SEL that identifies both the method on the
 * fromClass as well as the one on the toClass.
 *  @param fromClass The class from which a method is sourced.
 *  @param toClass The class to which the method is added. If the class already
 * has a method with the same selector, this has no effect.
 */
...
/** Copies a method identified by the sourceSelector from the fromClass as a
 * method for the destinationSelector on the toClass. After this method is
 * called, performing [toClassInstance destinationSelector] will be similar to
 * calling [fromClassInstance sourceSelector]. This method does nothing if
 * toClass already has a method identified by destinationSelector.
 *
 *  @param destinationSelector The SEL that identifies the method on the
 * toClass.
...
/** Enumerates through all the interceptors and if they respond to a given
 * selector, executes a GULAppDelegateInterceptorCallback with the interceptor.
 *
...
+ (void)notifyInterceptorsWithMethodSelector:(SEL)methodSelector
                                    callback:(GULAppDelegateInterceptorCallback)
                                                 callback {
  if (!callback) {
    return;
  }
  ...
  [interceptors enumerateKeysAndObjectsUsingBlock:^(id key, id obj,
                                                    BOOL *stop) {
    ...
    if (!interceptor) {
      GULLogWarning(
          ...
          [NSString stringWithFormat:
                        @"I-SWZ%06ld",
                        (long)kGULSwizzlerMessageCodeAppDelegateSwizzling010],
          @"AppDelegateProxy cannot find interceptor with ID %@. Removing the "
          @"interceptor.",
          key);
...
// The methods below are donor methods which are added to the dynamic subclass
// of the App Delegate. They are called within the scope of the real App
// Delegate so |self| does not refer to the GULAppDelegateSwizzler instance but
// the real App Delegate instance.
...
- (BOOL)application:(GULApplication *)application
  ...
  NSValue *openURLIMPPointer =
      [GULAppDelegateSwizzler originalImplementationForSelector:methodSelector
                                                         object:self];
...
#pragma clang diagnostic pop
  if (openURLOptionsIMP) {
    returnedValue |=
        openURLOptionsIMP(self, methodSelector, application, url, options);
  }
...
- (BOOL)application:(GULApplication *)application
  ...
  NSValue *openURLSourceAppAnnotationIMPPointer =
      [GULAppDelegateSwizzler originalImplementationForSelector:methodSelector
                                                         object:self];
  GULRealOpenURLSourceApplicationAnnotationIMP
      openURLSourceApplicationAnnotationIMP =
...
#pragma clang diagnostic pop
  ...
  if (openURLSourceApplicationAnnotationIMP) {
    returnedValue |= openURLSourceApplicationAnnotationIMP(
        self, methodSelector, application, url, sourceApplication, annotation);
  }
  return returnedValue;
}
...
#pragma clang diagnostic pop
  ...
  if (handleBackgroundSessionIMP) {
    handleBackgroundSessionIMP(self, methodSelector, application, identifier,
                               completionHandler);
  }
}
...
- (BOOL)application:(GULApplication *)application
  ...
  if (continueUserActivityIMP) {
    returnedValue |= continueUserActivityIMP(self, methodSelector, application,
...
- (void)application:(GULApplication *)application
  ...
  NSValue *didRegisterForRemoteNotificationsIMPPointer =
      [GULAppDelegateSwizzler originalImplementationForSelector:methodSelector
                                                         object:self];
  GULRealDidRegisterForRemoteNotificationsIMP
      didRegisterForRemoteNotificationsIMP =
  ...
  [GULAppDelegateSwizzler
      notifyInterceptorsWithMethodSelector:methodSelector
                                  callback:^(
                                    ...
                                    [invocation
                                        setArgument:(void *)(&application)
                                            atIndex:2];
                                    [invocation
                                        setArgument:(void *)(&deviceToken)
                                            atIndex:3];
                                    [invocation invoke];
                                  }];
  // Call the real implementation if the real App Delegate has any.
  if (didRegisterForRemoteNotificationsIMP) {
    didRegisterForRemoteNotificationsIMP(self, methodSelector, application,
                                         deviceToken);
  }
}
...
- (void)application:(GULApplication *)application
  ...
  NSValue *didFailToRegisterForRemoteNotificationsIMPPointer =
      [GULAppDelegateSwizzler originalImplementationForSelector:methodSelector
                                                         object:self];
  GULRealDidFailToRegisterForRemoteNotificationsIMP
      didFailToRegisterForRemoteNotificationsIMP =
      ...
      notifyInterceptorsWithMethodSelector:methodSelector
                                  callback:^(
                                    ...
                                    [invocation
                                        setArgument:(void *)(&application)
                                            atIndex:2];
                                    [invocation setArgument:(void *)(&error)
                                                    atIndex:3];
                                    [invocation invoke];
                                  }];
  // Call the real implementation if the real App Delegate has any.
  if (didFailToRegisterForRemoteNotificationsIMP) {
    didFailToRegisterForRemoteNotificationsIMP(self, methodSelector,
                                               application, error);
  }
}
...
- (void)application:(GULApplication *)application
      ...
      notifyInterceptorsWithMethodSelector:methodSelector
                                  callback:^(
                                    ...
                                    [invocation
                                        setArgument:(void *)(&application)
                                            atIndex:2];
                                    [invocation setArgument:(void *)(&userInfo)
                                                    atIndex:3];
                                    [invocation
                                        setArgument:(void *)(&completionHandler)
                                            atIndex:4];
                                    [invocation invoke];
                                  }];
  // Call the real implementation if the real App Delegate has any.
  if (didReceiveRemoteNotificationWithCompletionIMP) {
    didReceiveRemoteNotificationWithCompletionIMP(
        self, methodSelector, application, userInfo, completionHandler);
  }
}
#endif // !TARGET_OS_WATCH && !TARGET_OS_OSX
...
#pragma clang diagnostic pop
  ...
  if (didReceiveRemoteNotificationIMP) {
    didReceiveRemoteNotificationIMP(self, methodSelector, application,
                                    userInfo);
  }
}
...
+ (nullable NSInvocation *)appDelegateInvocationForSelector:(SEL)selector {
  struct objc_method_description methodDescription =
      protocol_getMethodDescription(@protocol(GULApplicationDelegate), selector,
                                    NO, YES);
...
+ (void)proxyAppDelegate:(id<GULApplicationDelegate>)appDelegate {
    ...
    GULLogNotice(
        ...
        [NSString
            stringWithFormat:
                @"I-SWZ%06ld",
                (long)
                    kGULSwizzlerMessageCodeAppDelegateSwizzlingInvalidAppDelegate],
        @"App Delegate does not conform to UIApplicationDelegate protocol. %@",
        [GULAppDelegateSwizzler
            correctAlternativeWhenAppDelegateProxyNotCreated]);
    return;
  }
  ...
  if (!originalDelegate) {
    GULLogError(
        ...
        @"Cannot create App Delegate Proxy because App Delegate instance is "
        @"nil. %@",
        [GULAppDelegateSwizzler
            correctAlternativeWhenAppDelegateProxyNotCreated]);
    return;
...
+ (NSString *)correctAppDelegateProxyKey {
  return NSClassFromString(@"FIRCore")
             ? kGULFirebaseAppDelegateProxyEnabledPlistKey
