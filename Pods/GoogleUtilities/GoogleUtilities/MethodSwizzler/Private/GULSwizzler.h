...
/** This class handles the runtime manipulation necessary to instrument
 * selectors. It stores the classes and selectors that have been swizzled, and
 * runs all operations on its own queue.
 */
 ...
 * declared as
 *  @dynamic, we have a reverse swizzling situation, where the implementation of
 * a method exists only in concrete subclasses, and NOT in the superclass. We
 * can detect that situation using this helper method. Similarly, we can detect
 * situations where a class doesn't implement a protocol method.
 *
 *  @param selector The selector to check for.
 *  @param aClass The class to check.
 *  @param isClassSelector A BOOL specifying whether the selector is a class or
 * instance selector.
 *  @return YES if the method was found in this selector/class combination, NO
 * otherwise.
 */
