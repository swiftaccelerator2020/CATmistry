...
- (void)setReachabilityApi:(const struct GULReachabilityApi *)reachabilityApi {
  ...
  if (reachability_) {
    GULLogError(
        ...
        [NSString stringWithFormat:@"I-REA%06ld",
                                   (long)kGULReachabilityMessageCode000],
...
- (void)setReachabilityDelegate:
  ...
  if (reachabilityDelegate &&
      (![(NSObject *)reachabilityDelegate
          conformsToProtocol:@protocol(GULReachabilityDelegate)])) {
    GULLogError(
        kGULLoggerReachability, NO,
...
- (instancetype)initWithReachabilityDelegate:
  ...
  if (!host || !host.length) {
    GULLogError(
        kGULLoggerReachability, NO,
        [NSString stringWithFormat:@"I-REA%06ld",
                                   (long)kGULReachabilityMessageCode001],
...
- (BOOL)start {
  ...
  if (!reachability_) {
    reachability_ = reachabilityApi_->createWithNameFn(kCFAllocatorDefault,
                                                       [host_ UTF8String]);
    if (!reachability_) {
      return NO;
    }
    SCNetworkReachabilityContext context = {
        0,                       /* version */
        (__bridge void *)(self), /* info (passed as last parameter to
                                    reachability callback) */
        NULL,                    /* retain */
        NULL,                    /* release */
        NULL                     /* copyDescription */
    };
    if (!reachabilityApi_->setCallbackFn(reachability_, ReachabilityCallback,
                                         &context) ||
        !reachabilityApi_->scheduleWithRunLoopFn(
            reachability_, CFRunLoopGetMain(), kCFRunLoopCommonModes)) {
      ...
      GULLogError(
          kGULLoggerReachability, NO,
          [NSString stringWithFormat:@"I-REA%06ld",
                                     (long)kGULReachabilityMessageCode002],
  ...
  GULLogDebug(kGULLoggerReachability, NO,
              [NSString stringWithFormat:@"I-REA%06ld",
                                         (long)kGULReachabilityMessageCode003],
...
// Connection required flag is not set, so we have connectivity.
      ...
      status = (flags & kSCNetworkReachabilityFlagsIsWWAN)
                   ? kGULReachabilityViaCellular
...
// If the connection on demand or connection on traffic flag is set, and user
// intervention is not required, we have connectivity.
#if TARGET_OS_IOS || TARGET_OS_TV
      status = (flags & kSCNetworkReachabilityFlagsIsWWAN)
                   ? kGULReachabilityViaCellular
                   : kGULReachabilityViaWifi;
...
- (void)reachabilityFlagsChanged:(SCNetworkReachabilityFlags)flags {
  ...
  if (reachabilityStatus_ != status) {
    ...
    GULLogDebug(
        kGULLoggerReachability, NO,
        [NSString stringWithFormat:@"I-REA%06ld",
                                   (long)kGULReachabilityMessageCode004],
