...
/**
 * The bundle ID for the application. Defaults to `[[NSBundle mainBundle]
 * bundleID]` when not set manually or in a plist.
 */
...
/**
 * The Android client ID used in Google AppInvite when an iOS app has its
...
/**
 * The App Group identifier to share data between the application and the
 * application extensions. The App Group must be configured in the application
 * and on the Apple Developer Portal. Default value `nil`.
 */
@property(nonatomic, copy, nullable) NSString *appGroupID;

/**
 * Initializes a customized instance of FIROptions from the file at the given
 * plist file path. This will read the file synchronously from disk. For
 * example, NSString *filePath =
 *     [[NSBundle mainBundle] pathForResource:@"GoogleService-Info"
 * ofType:@"plist"]; FIROptions *options = [[FIROptions alloc]
 * initWithContentsOfFile:filePath]; Returns nil if the plist file does not
 * exist or is invalid.
 */
- (nullable instancetype)initWithContentsOfFile:(NSString *)plistPath;

/**
 * Initializes a customized instance of FIROptions with required fields. Use the
 * mutable properties to modify fields for configuring specific services.
 */
// clang-format off
