...
// The kFIRService strings are only here while transitioning CoreDiagnostics
// from the Analytics pod to a Core dependency. These symbols are not used and
// should be deleted after the transition.
...
NSString *const kFIRAppReadyToConfigureSDKNotification =
...
NSString *const kFIRAppDiagnosticsNotification =
    @"FIRAppDiagnosticsNotification";
...
/**
 * An array of all classes that registered as `FIRCoreConfigurable` in order to
 * receive lifecycle events from Core.
 */
...
// This is necessary since our custom getter prevents `_options` from being
// created.
...
+ (void)configure {
  ...
  if (!options) {
    [NSException raise:kFirebaseCoreErrorDomain
                format:@"`[FIRApp configure];` (`FirebaseApp.configure()` in "
                       @"Swift) could not find "
  ...
  FIRLogNotice(
      kFIRLoggerCore, @"I-COR000028",
      @"tvOS and macOS SDK support is not part of the official Firebase "
      @"product. "
...
+ (NSCharacterSet *)applicationNameAllowedCharacters {
  ...
  dispatch_once(&onceToken, ^{
    NSMutableCharacterSet *allowedNameCharacters =
...
+ (void)configureWithName:(NSString *)name options:(FIROptions *)options {
  if (!name || !options) {
    [NSException raise:kFirebaseCoreErrorDomain
                format:@"Neither name nor options can be nil."];
  }
  if (name.length == 0) {
    [NSException raise:kFirebaseCoreErrorDomain
  ...
  if ([name isEqualToString:kFIRDefaultAppName]) {
    if (sDefaultApp) {
      // The default app already exists. Handle duplicate `configure` calls and
      // return.
      [self appWasConfiguredTwice:sDefaultApp usingOptions:options];
      return;
    }

    FIRLogDebug(kFIRLoggerCore, @"I-COR000001",
                @"Configuring the default app.");
  } else {
    // Validate the app name and ensure it hasn't been configured already.
    NSCharacterSet *nameCharacters =
        [NSCharacterSet characterSetWithCharactersInString:name];

    if (![[self applicationNameAllowedCharacters]
            isSupersetOfSet:nameCharacters]) {
      [NSException raise:kFirebaseCoreErrorDomain
    ...
    @synchronized(self) {
      if (sAllApps && sAllApps[name]) {
        // The app already exists. Handle a duplicate `configure` call and
        // return.
    ...
    FIRLogDebug(kFIRLoggerCore, @"I-COR000002", @"Configuring app named %@",
                name);
  }
  ...
  @synchronized(self) {
    ...
    // The FIRApp instance is ready to go, `sDefaultApp` is assigned, other SDKs
    // are now ready to be instantiated.
    [app.container instantiateEagerComponents];
    [FIRApp sendNotificationsToSDKs:app];
  }
}

/// Called when `configure` has been called multiple times for the same app.
/// This can either throw an exception (most cases) or ignore the duplicate
/// configuration in situations where it's allowed like an extension.
+ (void)appWasConfiguredTwice:(FIRApp *)app usingOptions:(FIROptions *)options {
  // Only extensions should potentially be able to call `configure` more than
  // once.
  ...
  // In an extension, the entry point could be called multiple times. As long as
  // the options are identical we should allow multiple `configure` calls.
  if ([options isEqual:app.options]) {
    // Everything is identical but the extension's lifecycle triggered
    // `configure` twice. Ignore duplicate calls and return since everything
...
+ (FIRApp *)defaultApp {
  ...
  FIRLogError(kFIRLoggerCore, @"I-COR000003",
              ...
              @"configured. Add `[FIRApp configure];` "
              @"(`FirebaseApp.configure()` in Swift) to your "
...
+ (FIRApp *)appNamed:(NSString *)name {
  @synchronized(self) {
    ...
    FIRLogError(kFIRLoggerCore, @"I-COR000004",
                @"App with name %@ does not exist.", name);
    return nil;
  }
}
...
+ (NSDictionary *)allApps {
  @synchronized(self) {
    if (!sAllApps) {
      FIRLogError(kFIRLoggerCore, @"I-COR000005",
...
- (void)deleteApp:(FIRAppVoidBoolCallback)completion {
  @synchronized([self class]) {
    if (sAllApps && sAllApps[self.name]) {
      FIRLogDebug(kFIRLoggerCore, @"I-COR000006", @"Deleting app named %@",
                  self.name);

      // Remove all registered libraries from the container to avoid creating
      // new instances.
      ...
      [[NSNotificationCenter defaultCenter]
          postNotificationName:kFIRAppDeleteNotification
                        object:[self class]
...
+ (void)addAppToAppDictionary:(FIRApp *)app {
  ...
  if ([app configureCore]) {
    ...
    [NSException
         raise:kFirebaseCoreErrorDomain
        format:@"Configuration fails. It may be caused by an invalid "
               @"GOOGLE_APP_ID in "
...
#if TARGET_OS_IOS
  // Initialize the Analytics once there is a valid options under default app.
  // Analytics should always initialize first by itself before the other SDKs.
  if ([self.name isEqualToString:kFIRDefaultAppName]) {
    Class firAnalyticsClass = NSClassFromString(@"FIRAnalytics");
    if (firAnalyticsClass) {
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wundeclared-selector"
      SEL startWithConfigurationSelector = @selector(startWithConfiguration:
                                                                    options:);
#pragma clang diagnostic pop
      if ([firAnalyticsClass
              respondsToSelector:startWithConfigurationSelector]) {
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Warc-performSelector-leaks"
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
        [firAnalyticsClass performSelector:startWithConfigurationSelector
                                withObject:[FIRConfiguration sharedInstance]
...
#ifdef DEBUG
  FIRLogDebug(kFIRLoggerCore, @"I-COR000034",
              @"Explicitly %@ data collection flag.",
              dataCollectionDefaultEnabled ? @"enabled" : @"disabled");
  self.alreadyOutputDataCollectionFlag = YES;
#endif // DEBUG

  NSString *key = [NSString
      stringWithFormat:kFIRGlobalAppDataCollectionEnabledDefaultsKeyFormat,
                       self.name];
  [[NSUserDefaults standardUserDefaults] setBool:dataCollectionDefaultEnabled
                                          forKey:key];

  // Core also controls the FirebaseAnalytics flag, so check if the Analytics
  // flags are set within FIROptions and change the Analytics value if
  // necessary. Analytics only works with the default app, so return if this
  // isn't the default app.
  if (!self.isDefaultApp) {
    return;
  }

  // Check if the Analytics flag is explicitly set. If so, no further actions
  // are necessary.
  if ([self.options isAnalyticsCollectionExplicitlySet]) {
    return;
  }

  // The Analytics flag has not been explicitly set, so update with the value
  // being set.
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
...
- (BOOL)isDataCollectionDefaultEnabled {
  // Check if it's been manually set before in code, and use that as the higher
  // priority value.
  NSNumber *defaultsObject =
      [[self class] readDataCollectionSwitchFromUserDefaultsForApp:self];
  if (defaultsObject != nil) {
#ifdef DEBUG
    if (!self.alreadyOutputDataCollectionFlag) {
      FIRLogDebug(kFIRLoggerCore, @"I-COR000031",
                  @"Data Collection flag is %@ in user defaults.",
  ...
  // Read the Info.plist to see if the flag is set. If it's not set, it should
  // default to `YES`. As per the implementation of
  // `readDataCollectionSwitchFromPlist`, it's a cached value and has no
  // performance impact calling multiple times.
  NSNumber *collectionEnabledPlistValue =
      [[self class] readDataCollectionSwitchFromPlist];
  if (collectionEnabledPlistValue != nil) {
#ifdef DEBUG
    if (!self.alreadyOutputDataCollectionFlag) {
      FIRLogDebug(kFIRLoggerCore, @"I-COR000032",
                  @"Data Collection flag is %@ in plist.",
                  [collectionEnabledPlistValue boolValue] ? @"enabled"
                                                          : @"disabled");
      self.alreadyOutputDataCollectionFlag = YES;
    }
#endif // DEBUG
...
#ifdef DEBUG
  if (!self.alreadyOutputDataCollectionFlag) {
    FIRLogDebug(kFIRLoggerCore, @"I-COR000033",
                @"Data Collection flag is not set.");
    self.alreadyOutputDataCollectionFlag = YES;
  }
#endif // DEBUG
  return YES;
...
+ (void)sendNotificationsToSDKs:(FIRApp *)app {
  // TODO: Remove this notification once all SDKs are registered with
  // `FIRCoreConfigurable`.
  ...
  [[NSNotificationCenter defaultCenter]
      postNotificationName:kFIRAppReadyToConfigureSDKNotification
                    object:self
                  userInfo:appInfoDict];
...
+ (NSError *)errorForMissingOptions {
  NSDictionary *errorDict = @{
    NSLocalizedDescriptionKey : @"Unable to parse GoogleService-Info.plist in "
                                @"order to configure services.",
...
+ (NSError *)errorForSubspecConfigurationFailureWithDomain:(NSString *)domain
  ...
  NSString *description = [NSString
      stringWithFormat:@"Configuration failed for service %@.", service];
  NSDictionary *errorDict = @{
...
+ (NSError *)errorForInvalidAppID {
  NSDictionary *errorDict = @{
    ...
    NSLocalizedRecoverySuggestionErrorKey :
        @"Check formatting and location of GoogleService-Info.plist or "
        @"GoogleAppID set in the "
        @"customized options."
  };
  return [NSError errorWithDomain:kFirebaseCoreErrorDomain
...
+ (void)registerLibrary:(nonnull NSString *)name
  ...
  // Create the set of characters which aren't allowed, only if this feature is
  // used.
  NSMutableCharacterSet *allowedSet =
      [NSMutableCharacterSet alphanumericCharacterSet];
  [allowedSet addCharactersInString:@"-_."];
  NSCharacterSet *disallowedSet = [allowedSet invertedSet];
  // Make sure the library name and version strings do not contain unexpected
  // characters, and add the name/version pair to the dictionary.
  if ([name rangeOfCharacterFromSet:disallowedSet].location == NSNotFound &&
    ...
    FIRLogError(kFIRLoggerCore, @"I-COR000027",
                @"The library name (%@) or version number (%@) contain invalid "
                @"characters. "
                @"Only alphanumeric, dash, underscore and period characters "
                @"are allowed.",
                name, version);
  }
}
...
+ (void)registerInternalLibrary:(nonnull Class<FIRLibrary>)library
  ...
  if (![(Class)library conformsToProtocol:@protocol(FIRLibrary)] ||
    ...
    [NSException
        ...
        format:@"Class %@ attempted to register components, but it does not "
               @"conform to "
...
+ (NSString *)firebaseUserAgent {
  @synchronized(self) {
    ...
    NSMutableArray<NSString *> *libraries = [[NSMutableArray<NSString *> alloc]
        initWithCapacity:sLibraryVersions.count];
...
+ (BOOL)hasSwiftRuntime {
  ...
  // is a part of Swift runtime, so it should be present if Swift runtime is
  // available.
...
+ (NSString *)applePlatform {
  ...
  // When a Catalyst app is run on macOS then both `TARGET_OS_MACCATALYST` and
  // `TARGET_OS_IOS` are `true`, which means the condition list is
  // order-sensitive.
...
- (void)checkExpectedBundleID {
  ...
  // The checking is only done when the bundle ID is provided in the serviceInfo
  // dictionary for backward compatibility.
  if (expectedBundleID != nil &&
      ![FIRBundleUtil hasBundleIdentifierPrefix:expectedBundleID
                                      inBundles:bundles]) {
    FIRLogError(kFIRLoggerCore, @"I-COR000008",
                @"The project's Bundle ID is inconsistent with "
                @"either the Bundle ID in '%@.%@', or the Bundle ID in the "
                @"options if you are "
                @"using a customized options. To ensure that everything can be "
                @"configured "
                @"correctly, you may need to make the Bundle IDs consistent. "
                @"To continue with this "
                @"plist file, you may change your app's bundle identifier to "
                @"'%@'. Or you can "
                @"download a new configuration file that matches your bundle "
                @"identifier from %@ "
                @"and replace the current one.",
                kServiceInfoFileName, kServiceInfoFileType, expectedBundleID,
                kPlistURL);
  }
}
...
/**
 * Validates the format and fingerprint of the app ID contained in GOOGLE_APP_ID
 * in the plist file. This is the main method for validating app ID.
 *
 * @return YES if the app ID fulfills the expected format and fingerprint, NO
 * otherwise.
 */
- (BOOL)isAppIDValid {
  ...
  if (!isValid) {
    ...
    FIRLogError(kFIRLoggerCore, @"I-COR000009",
                ...
                @"'%@.%@' or the one set in the customized options is invalid. "
                @"If you are using "
                @"the plist file, use the iOS version of bundle identifier to "
                @"download the file, "
                @"and do not manually edit the GOOGLE_APP_ID. You may change "
                @"your app's bundle "
                @"identifier to '%@'. Or you can download a new configuration "
                @"file that matches "
...
+ (BOOL)validateAppID:(NSString *)appID {
  // Failing validation only occurs when we are sure we are looking at a V2 app
  // ID and it does not have a valid fingerprint, otherwise we just warn about
  // the potential issue.
  if (!appID.length) {
    return NO;
  }
  ...
  if (![stringScanner
          scanCharactersFromSet:[NSCharacterSet decimalDigitCharacterSet]
...
/**
 * Validates that the format of the app ID string is what is expected based on
 * the supplied version. The version must end in ":".
 *
 * For v1 app ids the format is expected to be
 * '<version #>:<project number>:ios:<fingerprint of bundle id>'.
 *
 * This method does not verify that the contents of the app id are correct, just
 * that they fulfill the expected format.
 *
 * @param appID Contents of GOOGLE_APP_ID from the plist file.
 * @param version Indicates what version of the app id format this string should
 * be.
...
/**
 * Validates that the fingerprint of the app ID string is what is expected based
 * on the supplied version.
 *
 * Note that the v1 hash algorithm is not permitted on the client and cannot be
 * fully validated.
 *
 * @param appID Contents of GOOGLE_APP_ID from the plist file.
 * @param version Indicates what version of the app id format this string should
 * be.
 * @return YES if provided string fufills the expected fingerprint and the
 * version is known, NO otherwise.
 */
+ (BOOL)validateAppIDFingerprint:(NSString *)appID
                     withVersion:(NSString *)version {
  // Extract the supplied fingerprint from the supplied app ID.
  // This assumes the app ID format is the same for all known versions below. If
  // the app ID format changes in future versions, the tokenizing of the app ID
  // format will need to take into account the version of the app ID.
  ...
  if ([version isEqual:@"1"]) {
    // The v1 hash algorithm is not permitted on the client so the actual hash
    // cannot be validated.
    return YES;
  }
...
/**
 * Clears the data collection switch from the standard NSUserDefaults for easier
...
/**
 * Reads the data collection switch from the standard NSUserDefaults for easier
 * testing and readability.
 */
+ (nullable NSNumber *)readDataCollectionSwitchFromUserDefaultsForApp:
    (FIRApp *)app {
  // Read the object in user defaults, and only return if it's an NSNumber.
  NSString *key = [NSString
      stringWithFormat:kFIRGlobalAppDataCollectionEnabledDefaultsKeyFormat,
                       app.name];
  id collectionEnabledDefaultsObject =
...
/**
 * Reads the data collection switch from the Info.plist for easier testing and
...
+ (nullable NSNumber *)readDataCollectionSwitchFromPlist {
  ...
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    // Read the data from the `Info.plist`, only assign it if it's there and an
    // NSNumber.
