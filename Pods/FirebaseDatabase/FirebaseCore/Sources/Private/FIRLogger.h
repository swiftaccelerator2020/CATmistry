 ...
 * If set to YES, the logging level for Analytics will be set to
 * FIRLoggerLevelDebug. Enabling the debug mode has no effect if the app is
 * running from App Store. (required) analytics debug mode flag.
 */
 ...
 * (required) whether or not this function is called from the Analytics
 * component.
 */
BOOL FIRIsLoggableLevel(FIRLoggerLevel loggerLevel, BOOL analyticsComponent);

/**
 * Logs a message to the Xcode console and the device log. If running from
 * AppStore, will not log any messages with a level higher than
 * FIRLoggerLevelNotice to avoid log spamming. (required) log level (one of the
 * FIRLoggerLevel enum values). (required) service name of type
 * FIRLoggerService. (required) message code starting with "I-" which means iOS,
 * followed by a capitalized three-character service identifier and a six digit
 * integer message ID that is unique within the service. An example of the
 * message code is @"I-COR000001". (required) message string which can be a
 * format string. (optional) variable arguments list obtained from calling
 * va_start, used when message is a format string.
 */
extern void FIRLogBasic(FIRLoggerLevel level, FIRLoggerService service,
 ...
 * (required) message code starting from "I-" which means iOS, followed by a
 ...
 * is @"I-COR000001". See go/firebase-log-proposal for details. (required)
 * message string which can be a format string. (optional) the list of arguments
 * to substitute into the format string. Example usage:
 * FIRLogError(kFIRLoggerCore, @"I-COR000001", @"Configuration of %@ failed.",
 * app.name);
 */
extern void FIRLogError(FIRLoggerService service, NSString *messageCode,
                        NSString *message, ...) NS_FORMAT_FUNCTION(3, 4);
extern void FIRLogWarning(FIRLoggerService service, NSString *messageCode,
                          NSString *message, ...) NS_FORMAT_FUNCTION(3, 4);
extern void FIRLogNotice(FIRLoggerService service, NSString *messageCode,
                         NSString *message, ...) NS_FORMAT_FUNCTION(3, 4);
extern void FIRLogInfo(FIRLoggerService service, NSString *messageCode,
                       NSString *message, ...) NS_FORMAT_FUNCTION(3, 4);
extern void FIRLogDebug(FIRLoggerService service, NSString *messageCode,
                        NSString *message, ...) NS_FORMAT_FUNCTION(3, 4);
