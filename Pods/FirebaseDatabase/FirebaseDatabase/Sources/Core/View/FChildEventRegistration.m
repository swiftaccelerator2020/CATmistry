...
- (BOOL)responseTo:(FIRDataEventType)eventType {
  return self.callbacks != nil &&
         [self.callbacks objectForKey:[NSNumber numberWithInteger:eventType]] !=
             nil;
}
...
- (FDataEvent *)createEventFrom:(FChange *)change query:(FQuerySpec *)query {
  ...
  FIRDataSnapshot *snapshot =
      [[FIRDataSnapshot alloc] initWithRef:ref indexedNode:change.indexedNode];

  FDataEvent *eventData = [[FDataEvent alloc] initWithEventType:change.type
...
- (void)fireEvent:(id<FEvent>)event queue:(dispatch_queue_t)queue {
  if ([event isCancelEvent]) {
    ...
    NSAssert(self.cancelCallback != nil,
