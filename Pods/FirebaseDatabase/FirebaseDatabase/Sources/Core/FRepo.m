...
- (id)initWithRepoInfo:(FRepoInfo *)info
    ...
    if (self) {
        ...
        // initialized here
        self.serverClock = [[FOffsetClock alloc] initWithClock:[FSystemClock clock]
                                                 offset:0];
...
- (void)deferredInit {
    ...
    if (self.config.persistenceEnabled) {
        NSString *repoHashString = [NSString
                                    stringWithFormat:@"%@_%@", self.repoInfo.host, self.repoInfo.namespace];
...
- (void)restoreWrites {
    ...
    [writes enumerateObjectsUsingBlock:^(FWriteRecord *write, NSUInteger idx,
           BOOL *stop) {
               ...
               fbt_void_nsstring_nsstring callback = ^(NSString *status,
...
- (void)set:(FPath *)path
    ...
    id<FNode> existing = [self.serverSyncTree calcCompleteEventCacheAtPath:path
                                              excludeWriteIds:@[]];
    id<FNode> newNode = [FServerValues resolveDeferredValueSnapshot:node
...
- (void)onDisconnectCancel:(FPath *)path
    ...
    withCallback:^(NSString *status, NSString *errorReason) {
        ...
        if (success) {
            ...
            FFLog(@"I-RDB038007",
                  @"cancelDisconnectOperations: at %@ failed: %@", path,
                  status);
        }
...
- (void)onDataUpdate:(FPersistentConnection *)fpconnection
    ...
    FFLog(@"I-RDB038013", @"onDataUpdateForPath: %@ withMessage: %@", pathString,
          data);
    ...
    if (tagId != nil) {
        if (isMerge) {
            ...
            events = [self.serverSyncTree applyTaggedQueryMergeAtPath:path
                                          changedChildren:taggedChildren
                                          tagId:tagId];
        } else {
            id<FNode> taggedSnap = [FSnapshotUtilities nodeFrom:data];
            events = [self.serverSyncTree applyTaggedQueryOverwriteAtPath:path
                                          newData:taggedSnap
                                          tagId:tagId];
        }
    } else if (isMerge) {
    ...
    if (!result.isSuccess) {
        ...
        if (transaction.onComplete) {
            ...
            FIndexedNode *indexedNode =
                [FIndexedNode indexedNodeWithNode:transaction.currentInputSnapshot];
            FIRDataSnapshot *snap = [[FIRDataSnapshot alloc] initWithRef:ref
...
- (void)sendTransactionQueue:(NSMutableArray *)queue atPath:(FPath *)path {
    ...
    [self.connection
     ...
     withHash:latestHash
    withCallback:^(NSString *status, NSString *errorReason) {
        FFLog(@"I-RDB038022", @"Transaction put response: %@ : %@", pathToSend,
              status);
        ...
        if ([status isEqualToString:kFWPResponseForActionStatusOk]) {
            ...
            if ([status isEqualToString:kFWPResponseForActionStatusDataStale]) {
                ...
                FFWarn(@"I-RDB038023", @"runTransactionBlock: at %@ failed: %@",
                       path, status);
                for (FTupleTransaction *transaction in queue) {
                    transaction.status = FTransactionNeedsAbort;
...
- (void)rerunTransactionQueue:(NSArray *)queue atPath:(FPath *)path {
    ...
    for (FTupleTransaction *transaction in queue) {
        FPath *relativePath __unused = [FPath relativePathFrom:path
                                              to:transaction.path];
        ...
        if (transaction.status == FTransactionNeedsAbort) {
            ...
            if (![transaction.abortStatus isEqualToString:kFErrorWriteCanceled]) {
                NSArray *ackEvents = [self.serverSyncTree
                                      ackUserWriteWithWriteId:[transaction.currentWriteId integerValue]
                                      revert:YES
                                      persist:NO
                                      clock:self.serverClock];
            ...
            if (transaction.retryCount >= kFTransactionMaxRetries) {
                ...
                id<FNode> currentNode = [self latestStateAtPath:transaction.path
                                              excludeWriteIds:writeIdsToExclude];
                ...
                if (result.isSuccess) {
                    ...
                    id<FNode> newValResolved = [FServerValues
                                                ...
                                                withExisting:transaction.currentInputSnapshot
                                                serverValues:serverValues];
                    ...
                    [events
                     ...
                     [self.serverSyncTree
                      ...
                      writeId:[transaction.currentWriteId
                               integerValue]
                      isVisible:transaction.applyLocally]];
                    [events addObjectsFromArray:
                            [self.serverSyncTree
                             ackUserWriteWithWriteId:[oldWriteId integerValue]
                             revert:YES
                             persist:NO
                             clock:self.serverClock]];
                } else {
