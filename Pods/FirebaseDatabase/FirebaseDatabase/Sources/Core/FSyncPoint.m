...
- (NSArray *)applyOperation:(id<FOperation>)operation
  ...
  if (!view.query.loadsAllData) {
    ...
    [result.changes enumerateObjectsUsingBlock:^(FChange *change,
    ...
    if ([removed count] > 0 || [added count] > 0) {
      [self.persistenceManager updateTrackedQueryKeysWithAddedKeys:added
                                                       removedKeys:removed
                                                          forQuery:view.query];
    }
  }
...
- (NSArray *)addEventRegistration:(id<FEventRegistration>)eventRegistration
  ...
  FCacheNode *eventCacheNode =
      [[FCacheNode alloc] initWithIndexedNode:indexed
                           isFullyInitialized:eventCacheComplete
                                   isFiltered:NO];
  FViewCache *viewCache = [[FViewCache alloc] initWithEventCache:eventCacheNode
  ...
  if (!query.loadsAllData) {
    ...
    [view.eventCache enumerateChildrenUsingBlock:^(NSString *key,
...
- (FTupleRemovedQueriesEvents *)removeEventRegistration:
  ...
  if ([query isDefault]) {
    ...
    [self.views enumerateKeysAndObjectsUsingBlock:^(
                    FQueryParams *viewQueryParams, FView *view, BOOL *stop) {
      [cancelEvents
          addObjectsFromArray:[view removeEventRegistration:eventRegistration
  ...
  return
      [[FTupleRemovedQueriesEvents alloc] initWithRemovedQueries:removedQueries
