...
- (void)onDisconnect:(FConnection *)fconnection
    ...
    FFLog(@"I-RDB034004", @"Got on disconnect due to %s",
          (reason == DISCONNECT_REASON_SERVER_RESET) ? "server_reset" : "other");
    connectionState = ConnectionStateDisconnected;
    ...
    if ([self shouldReconnect]) {
        NSTimeInterval timeSinceLastConnectSucceeded =
            [[NSDate date] timeIntervalSince1970] - lastConnectionEstablishedTime;
        ...
        [self.retryHelper retry:^ {
            ...
            withCallback:^(NSString *token, NSError *error) {
                if (thisFetchTokenAttempt ==
                    ...
                    if (error != nil) {
                        self->connectionState = ConnectionStateDisconnected;
                        FFLog(@"I-RDB034010", @"Error fetching token: %@",
                              error);
                        [self tryScheduleReconnect];
                    } else {
    ...
    [self sendAction:kFWPRequestActionAuth
        ...
        if (statusOk) {
            ...
            if ([status isEqualToString:@"expired_token"]) {
                FFLog(@"I-RDB034017", @"Authentication failed: %@ (%@)", status,
                      responseData);
            } else {
...
kFWPRequestCompoundHashHashes :
            compoundHash.hashes,
kFWPRequestCompoundHashPaths :
            posts
        };
    }
    ...
    if (payload != nil && [payload isKindOfClass:[NSDictionary class]]) {
        ...
        if (warnings != nil && [warnings isKindOfClass:[NSArray class]]) {
            ...
            if ([warningsArr containsObject:@"no_index"]) {
                NSString *indexSpec =
                    [NSString stringWithFormat:@"\".indexOn\": \"%@\"",
...
- (void)sendAction:(NSString *)action
    ...
    NSDictionary *msg =
        [NSDictionary dictionaryWithObjectsAndKeys:rn, kFWPRequestNumber, action,
...
- (void)cancelSentTransactions {
    NSMutableDictionary<NSNumber *, FOutstandingPut *> *cancelledOutstandingPuts =
...
- (void)onDataPushWithAction:(NSString *)action andBody:(NSDictionary *)body {
    ...
    if ([action isEqualToString:kFWPAsyncServerDataUpdate] ||
        ...
        if ([body objectForKey:kFWPAsyncServerDataUpdateBodyPath] &&
            ...
            if (isMerge && [payloadData isKindOfClass:[NSDictionary class]] &&
                ...
                [delegate
                 ...
                 message:payloadData
                 isMerge:isMerge
                 tagId:[body objectForKey:kFWPAsyncServerDataUpdateBodyTag]];
            }
        } else {
            FFLog(@"I-RDB034030",
        ...
        for (NSDictionary *range in ranges) {
            ...
            FPath *start =
                (startString != nil) ? [[FPath alloc] initWith:startString] : nil;
            FPath *end =
...
- (void)restoreState {
    ...
    [self.listens
     enumerateKeysAndObjectsUsingBlock:^(
    FQuerySpec *query, FOutstandingQuery *outstandingListen, BOOL *stop) {
...
- (void)handleTimestamp:(NSNumber *)timestamp {
    ...
    double timestampDeltaMs =
        [timestamp doubleValue] - ([[NSDate date] timeIntervalSince1970] * 1000);
    [self.delegate onServerInfoUpdate:self
                  updates:@ {
              kDotInfoServerTimeOffset :
                      [NSNumber numberWithDouble:timestampDeltaMs]
                  }];
}
