  ...
  default:
    @throw [[NSException alloc]
        initWithName:@"KnownEventTypeValidation"
              reason:[NSString stringWithFormat:@"(%@) Unknown event type: %d",
                                                fn, (int)event]
            userInfo:nil];
    break;
  }
...
+ (void)validateFrom:(NSString *)fn validRootPathString:(NSString *)pathString {
  ...
  dispatch_once(&token, ^{
    dotInfoRegex =
        [NSRegularExpression regularExpressionWithPattern:@"^\\/*\\.info(\\/|$)"
                                                  options:0
                                                    error:nil];
  });
    ...
    if ([theString maximumLengthOfBytesUsingEncoding:NSUTF8StringEncoding] >
      ...
      @throw [[NSException alloc]
          initWithName:@"InvalidFirebaseData"
                reason:[NSString stringWithFormat:@"(%@) String exceeds max "
    ...
    if (dval[kServerValueSubKey] != nil) {
      if ([dval count] > 1) {
        ...
        @throw [[NSException alloc]
            initWithName:@"InvalidFirebaseData"
                  reason:[NSString
                             stringWithFormat:@"(%@) Cannot store other keys "
  ...
  if ([value isKindOfClass:[NSNumber class]]) {
    if ([[NSDecimalNumber notANumber] isEqualToNumber:value]) {
      if (throwError) {
        ...
        @throw [[NSException alloc]
            initWithName:@"InvalidFirebaseData"
                  reason:[NSString stringWithFormat:@"(%@) Cannot store NaN as "
                                                    @"priority at path: %@.",
                                                    fn, pathString]
                userInfo:nil];
      } else {
      ...
      if (throwError) {
        ...
        @throw [[NSException alloc]
            initWithName:@"InvalidFirebaseData"
                  reason:[NSString
                             stringWithFormat:@"(%@) Cannot store true/false "
                                              @"as priority at path: %@.",
                                              fn, pathString]
                userInfo:nil];
      } else {
    ...
    if (throwError) {
      ...
      @throw [[NSException alloc]
          initWithName:@"InvalidFirebaseData"
                reason:[NSString
                           stringWithFormat:@"(%@) Cannot store an NSArray as "
                                            @"priority at path: %@.",
                                            fn, pathString]
              userInfo:nil];
    } else {
