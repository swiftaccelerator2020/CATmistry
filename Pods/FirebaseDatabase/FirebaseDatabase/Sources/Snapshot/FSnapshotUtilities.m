...
+ (id<FNode>)internalNodeFrom:(id)val
        ...
        if (dict[kPayloadValue] != nil) {
            ...
            if ([FValidation validateFrom:fn isValidLeafValue:value withPath:path]) {
                ...
                @throw [[NSException alloc]
                        ...
                        reason:[NSString
                                stringWithFormat:@"(%@) Invalid data type used "
    ...
    if ([value isKindOfClass:[NSDictionary class]]) {
        ...
        if ([children count] == 0) {
            ...
            FImmutableSortedDictionary *childrenDict = [FImmutableSortedDictionary
        ...
        for (int i = 0; i < [aval count]; i++) {
            ...
            [path addObject:key];
            id<FNode> childNode = [FSnapshotUtilities nodeFrom:[aval objectAtIndex:i]
                                                      priority:nil
                                                      withValidationFrom:fn
                                                      atDepth:depth + 1
                                                      path:path];
...
+ (void)appendHashRepresentationForLeafNode:(FLeafNode *)leafNode
    ...
    if (!leafNode.getPriority.isEmpty) {
        ...
        [FSnapshotUtilities appendHashRepresentationForLeafNode:leafNode.getPriority
                            toString:string
...
+ (NSUInteger)estimateSerializedNodeSize:(id<FNode>)node {
    if ([node isEmpty]) {
        ...
        [((FChildrenNode *)node) enumerateChildrenAndPriorityUsingBlock:^(
                                NSString *key, id<FNode> child, BOOL *stop) {
                                    sum += key.length;
