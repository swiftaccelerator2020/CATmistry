...
- (NSString *)dataHash {
  if (self.lazyHash == nil) {
    ...
    if (!self.getPriority.isEmpty) {
      ...
      [FSnapshotUtilities
          appendHashRepresentationForLeafNode:(FLeafNode *)self.getPriority
                                     toString:toHash
    ...
    if (sawPriority) {
      ...
      [array sortUsingComparator:^NSComparisonResult(FNamedNode *namedNode1,
                                                     FNamedNode *namedNode2) {
        return [[FPriorityIndex priorityIndex] compareNamedNode:namedNode1
                                                    toNamedNode:namedNode2];
      }];
      [array enumerateObjectsUsingBlock:^(FNamedNode *namedNode, NSUInteger idx,
                                          BOOL *stop) {
...
- (FNamedNode *)firstChild {
  ...
  if (childKey) {
    return [[FNamedNode alloc] initWithName:childKey
