...
- (void)ensureIndexed {
  if (!self.indexed) {
    if ([self.index isEqual:[FKeyIndex keyIndex]]) {
      ...
      [self.node enumerateChildrenUsingBlock:^(NSString *key, id<FNode> node,
                                               BOOL *stop) {
...
- (FIndexedNode *)updateChild:(NSString *)key
  ...
  if (self.indexed == [FIndexedNode fallbackIndex] &&
    ...
    FImmutableSortedSet *newIndexed =
        [self.indexed removeObject:[FNamedNode nodeWithName:key node:oldChild]];
...
- (void)enumerateChildrenReverse:(BOOL)reverse
  ...
  if (self.indexed == [FIndexedNode fallbackIndex]) {
    ...
    [self.indexed enumerateObjectsReverse:reverse
