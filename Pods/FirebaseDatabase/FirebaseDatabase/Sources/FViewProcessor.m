...
- (FViewProcessorResult *)applyOperationOn:(FViewCache *)oldViewCache
    ...
    if (operation.type == FOperationTypeOverwrite) {
        ...
        if (operation.source.fromUser) {
            ...
            BOOL filterServerNode =
                overwrite.source.isTagged ||
                (oldViewCache.cachedServerSnap.isFiltered && !overwrite.path.isEmpty);
        ...
        if (operation.source.fromUser) {
            ...
            // previously filtered
            BOOL filterServerNode =
                merge.source.isTagged || oldViewCache.cachedServerSnap.isFiltered;
...
- (FViewCache *)
    ...
    if ([writesCache shadowingWriteAtPath:changePath] != nil) {
        ...
        if (changePath.isEmpty) {
            ...
            NSAssert(viewCache.cachedServerSnap.isFullyInitialized,
            ...
            if (viewCache.cachedServerSnap.isFiltered) {
                ...
                nodeWithLocalWrites =
                    [writesCache calculateCompleteEventCacheWithCompleteServerCache:
            ...
            if ([childKey isEqualToString:@".priority"]) {
                ...
                id<FNode> updatedPriority = [writesCache
                                             calculateEventCacheAfterServerOverwriteWithChildPath:changePath
                                             existingEventSnap:oldEventNode
                                             existingServerSnap:serverNode];
                if (updatedPriority != nil) {
                    newEventCache = [self.filter updatePriority:updatedPriority
                ...
                if ([oldEventSnap isCompleteForChild:childKey]) {
                    ...
                    id<FNode> eventChildUpdate = [writesCache
                                                  calculateEventCacheAfterServerOverwriteWithChildPath:changePath
                                                  existingEventSnap:oldEventSnap
                                                  .node
                                                  existingServerSnap:serverNode];
...
- (FViewCache *)applyServerOverwriteTo:(FViewCache *)oldViewCache
    ...
    if (changePath.isEmpty) {
        FIndexedNode *indexed =
            [FIndexedNode indexedNodeWithNode:changedSnap index:serverFilter.index];
        ...
        FIndexedNode *indexed = [oldServerSnap.indexedNode updateChild:childKey
        ...
        id<FNode> newChildNode = [childNode updateChild:childChangePath
                                            withNewChild:changedSnap];
        if ([childKey isEqualToString:@".priority"]) {
            newServerCache = [serverFilter updatePriority:newChildNode
    ...
    FViewCache *newViewCache = [oldViewCache
                                updateServerSnap:newServerCache
...
- (FViewCache *)applyUserOverwriteTo:(FViewCache *)oldViewCache
    ...
    if (changePath.isEmpty) {
        FIndexedNode *newIndexed =
            [FIndexedNode indexedNodeWithNode:changedSnap index:self.filter.index];
        FIndexedNode *newEventCache =
        ...
        if ([childKey isEqualToString:@".priority"]) {
            FIndexedNode *newEventCache =
                [self.filter updatePriority:changedSnap
            ...
            if (childChangePath.isEmpty) {
                ...
                if (childNode != nil) {
                    if ([[childChangePath getBack] isEqualToString:@".priority"] &&
            ...
            if (![oldChild isEqual:newChild]) {
                FIndexedNode *newEventSnap =
                    [self.filter updateChildIn:oldEventSnap.indexedNode
                                 ...
                                 accumulator:accumulator];
                newViewCache =
                    [oldViewCache updateEventSnap:newEventSnap
                                  isComplete:oldEventSnap.isFullyInitialized
                                  isFiltered:self.filter.filtersNodes];
            } else {
...
- (FViewCache *)applyUserMergeTo:(FViewCache *)viewCache
    ...
    [changedChildren
    enumerateWrites:^(FPath *relativePath, id<FNode> childNode, BOOL *stop) {
...
- (FViewCache *)applyServerMergeTo:(FViewCache *)viewCache
    ...
    if (path.isEmpty) {
        ...
        actualMerge = [[FCompoundWrite emptyWrite] addCompoundWrite:changedChildren
                                                   atPath:path];
    }
    id<FNode> serverNode = viewCache.cachedServerSnap.node;
...
- (FViewCache *)ackUserWriteOn:(FViewCache *)viewCache
    ...
    if (affectedTree.value != nil) {
        ...
        if ((ackPath.isEmpty && serverCache.isFullyInitialized) ||
            ...
            return [self applyServerOverwriteTo:viewCache
                         changePath:ackPath
...
- (FViewCache *)revertUserWriteOn:(FViewCache *)viewCache
    ...
    if ([writesCache shadowingWriteAtPath:path] != nil) {
        ...
        if (path.isEmpty || [[path getFront] isEqualToString:@".priority"]) {
            ...
            FIndexedNode *indexedNode =
                [FIndexedNode indexedNodeWithNode:newNode index:self.filter.index];
...
- (FViewCache *)listenCompleteOldCache:(FViewCache *)viewCache
    ...
    return
        [self generateEventCacheAfterServerEvent:newViewCache
              path:path
              writesCache:writesCache
