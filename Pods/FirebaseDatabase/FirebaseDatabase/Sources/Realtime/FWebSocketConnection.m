...
- (void)open {
    ...
    dispatch_time_t when =
        dispatch_time(DISPATCH_TIME_NOW, kWebsocketConnectTimeout * NSEC_PER_SEC);
...
- (void)send:(NSDictionary *)dictionary {
    ...
    if (dataSegs.count > 1) {
        [self.webSocket
         send:[NSString stringWithFormat:@"%u", (unsigned int)dataSegs.count]];
    }
...
- (void)handleNewFrameCount:(int)numFrames {
    ...
    FFLog(@"I-RDB083006", @"(wsc:%@) handleNewFrameCount: %d", self.connectionId,
          self.totalFrames);
}
...
- (void)webSocket:(FSRWebSocket *)webSocket
    ...
    FFLog(@"I-RDB083011", @"(wsc:%@) didCloseWithCode: %ld %@", self.connectionId,
          (long)code, reason);
    [self onClosed];
}
