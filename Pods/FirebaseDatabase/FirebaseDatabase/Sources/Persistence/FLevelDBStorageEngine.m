...
- (void)runMigration {
  ...
  NSError *error;
  NSString *oldVersion = [NSString stringWithContentsOfFile:versionFile
  ...
  if (!oldVersion) {
    ...
    FFWarn(@"I-RDB076036", @"Version file empty. Assuming database version 1.");
  } else {
...
- (void)runLegacyMigration:(FRepoInfo *)info {
  ...
  if ([[NSFileManager defaultManager] fileExistsAtPath:legacyBaseDir]) {
    ...
    APLevelDB *writes = [APLevelDB
        levelDBWithPath:[legacyBaseDir
                            stringByAppendingPathComponent:@"outstanding_puts"]
                  error:&error];
    if (writes != nil) {
      ...
      [writes enumerateKeysAndValuesAsData:^(NSString *key, NSData *data,
        ...
        if ([pendingPut isKindOfClass:[FPendingPut class]]) {
          ...
          id<FNode> newNode = [FSnapshotUtilities nodeFrom:put.data
          ...
          id<FNode> newNode = [FSnapshotUtilities nodeFrom:putPriority.priority
                                                  priority:nil];
          [self saveUserOverwrite:newNode
                           atPath:priorityPath
                          writeId:[key integerValue]];
          ...
          FCompoundWrite *merge =
              [FCompoundWrite compoundWriteWithValueDictionary:update.data];
...
- (APLevelDB *)createDB:(NSString *)dbName {
  ...
  if (err) {
    ...
    if (err) {
      NSString *reason =
          [NSString stringWithFormat:@"Error initializing persistence: %@",
                                     [err description]];
      @throw
          [NSException exceptionWithName:@"FirebaseDatabasePersistenceFailure"
                                  reason:reason
                                userInfo:nil];
    }
  }
...
- (void)saveUserOverwrite:(id<FNode>)node
  ...
  NSData *data = [NSJSONSerialization dataWithJSONObject:write
                                                 options:0
                                                   error:&error];
  NSAssert(data, @"Failed to serialize user overwrite: %@, (Error: %@)", write,
           error);
...
- (void)removeAllUserWrites {
  ...
  if (!success) {
    ...
    FFDebug(@"I-RDB076011", @"Removed %lu writes in %fms", (unsigned long)count,
...
- (void)updateServerCache:(id<FNode>)node
  ...
  if (merge) {
    ...
    [node enumerateChildrenUsingBlock:^(NSString *childKey, id<FNode> childNode,
                                        BOOL *stop) {
  ...
  [self.serverCacheDB
      enumerateKeysWithPrefix:prefix
                   usingBlock:^(NSString *dbKey, BOOL *stop) {
                     ...
                     if ([pruneForest
                             shouldPruneUnkeptDescendantsAtPath:relativePath]) {
                       pruned++;
      ...
      enumerateKeysWithPrefix:kFTrackedQueriesPrefix
                       asData:^(NSString *key, NSData *data, BOOL *stop) {
                         ...
                         if (queryJSON == nil) {
                           ...
                           FPath *path = [FPath
                               pathWithString:queryJSON[kFTrackedQueryPath]];
                           FQueryParams *params = [FQueryParams
                               fromQueryObject:queryJSON[kFTrackedQueryParams]];
                           FQuerySpec *query =
                               [[FQuerySpec alloc] initWithPath:path
                                                         params:params];
                           BOOL isComplete =
                               [queryJSON[kFTrackedQueryIsComplete] boolValue];
                           BOOL isActive =
                               [queryJSON[kFTrackedQueryIsActive] boolValue];
                           NSTimeInterval lastUse =
                               [queryJSON[kFTrackedQueryLastUse] doubleValue];

                           FTrackedQuery *trackedQuery =
                               [[FTrackedQuery alloc] initWithId:queryId
                                                           query:query
                                                         lastUse:lastUse
                                                        isActive:isActive
...
- (void)removeTrackedQuery:(NSUInteger)queryId {
  ...
  [self.serverCacheDB enumerateKeysWithPrefix:trackedQueryKeysKeyPrefix(queryId)
  ...
  if (markAsDoNotBackup) {
    ...
    if (!success) {
      FFWarn(@"I-RDB076035",
             ...
             error);
      [NSException
           raise:@"Error marking as do not backup"
          format:@"Failed to mark folder %@ as do not backup", firebaseDirURL];
    }
  }
}
