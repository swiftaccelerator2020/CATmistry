...
- (id)initWithStorageEngine:(id<FStorageEngine>)storageEngine
    ...
    if (self != nil) {
        ...
        [trackedQueries enumerateObjectsUsingBlock:^(FTrackedQuery *trackedQuery,
...
- (void)setQueriesCompleteAtPath:(FPath *)path {
    ...
    forEach:^(FPath *childPath, NSDictionary *trackedQueries) {
        ...
        FQueryParams *parms, FTrackedQuery *trackedQuery, BOOL *stop) {
...
- (BOOL)hasActiveDefaultQueryAtPath:(FPath *)path {
    ...
    matching:^BOOL(NSDictionary *trackedQueries) {
        return [trackedQueries[[FQueryParams defaultInstance]]
                isActive];
    }] != nil;
}
...
- (void)cacheTrackedQuery:(FTrackedQuery *)query {
    ...
    if (trackedDict == nil) {
        ...
        self.trackedQueryTree = [self.trackedQueryTree setValue:trackedDict
...
- (FPruneForest *)pruneOldQueries:(id<FCachePolicy>)cachePolicy {
    ...
    [self.trackedQueryTree forEach:^(FPath *path, NSDictionary *trackedQueries) {
                              [trackedQueries
                               enumerateKeysAndObjectsUsingBlock:^(
                              FQueryParams *params, FTrackedQuery *trackedQuery, BOOL *stop) {
                                  if (!trackedQuery.isActive) {
    ...
    [pruneableQueries sortUsingComparator:^NSComparisonResult(FTrackedQuery *q1,
                     FTrackedQuery *q2) {
    ...
    NSUInteger numToPrune = [self numberOfQueriesToPrune:cachePolicy
    ...
    [unpruneableQueries enumerateObjectsUsingBlock:^(FTrackedQuery *toKeep,
                       NSUInteger idx, BOOL *stop) {
                           pruneForest = [pruneForest keepPath:toKeep.query.path];
                       }];
