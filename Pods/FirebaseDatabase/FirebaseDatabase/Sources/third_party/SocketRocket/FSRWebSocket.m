...
- (FSRIOConsumer *)consumerWithScanner:(stream_scanner)scanner
...
{
    assert(dispatch_get_specific((__bridge void *)self) ==
           maybe_bridge(_workQueue));
}
...
{
    assert(_url);
    NSAssert(_readyState == SR_CONNECTING,
...
- (BOOL)_checkHandshake:(CFHTTPMessageRef)httpMessage;
{
    NSString *acceptHeader = CFBridgingRelease(CFHTTPMessageCopyHeaderFieldValue(
    ...
    NSString *concattedString =
        [_secKey stringByAppendingString:SRWebSocketAppendToSecKeyString];
...
- (void)_HTTPHeadersDidFinish;
{
    NSInteger responseCode =
        CFHTTPMessageGetResponseStatusCode(_receivedHTTPHeaders);
    ...
    if (![self _checkHandshake:_receivedHTTPHeaders]) {
        [self _failWithError:
              [NSError
               errorWithDomain:FSRWebSocketErrorDomain
               code:2133
               userInfo:
               [NSDictionary
                dictionaryWithObject:
    ...
    NSString *negotiatedProtocol =
        CFBridgingRelease(CFHTTPMessageCopyHeaderFieldValue(
                              _receivedHTTPHeaders, CFSTR("Sec-WebSocket-Protocol")));
...
- (void)_readHTTPHeader;
{
    ...
    [self
    _readUntilHeaderCompleteWithCallback:^(FSRWebSocket *self, NSData *data) {
        CFHTTPMessageAppendBytes(self->_receivedHTTPHeaders,
                                 (const UInt8 *)data.bytes, data.length);

        if (CFHTTPMessageIsHeaderComplete(self->_receivedHTTPHeaders)) {
            SRFastLog(@"Finished reading headers %@",
                      CFBridgingRelease(CFHTTPMessageCopyAllHeaderFields(
                                            self->_receivedHTTPHeaders)));
...
- (void)didConnect {
    ...
    CFHTTPMessageRef request = CFHTTPMessageCreateRequest(
                                   NULL, CFSTR("GET"), (__bridge CFURLRef)_url, kCFHTTPVersion1_1);
    ...
    CFHTTPMessageSetHeaderFieldValue(
        ...
        (__bridge CFStringRef)(
            _url.port != nil
            ? [NSString stringWithFormat:@"%@:%@", _url.host, _url.port]
            : _url.host));

    NSMutableData *keyBytes = [[NSMutableData alloc] initWithLength:16];
    int result = SecRandomCopyBytes(kSecRandomDefault, keyBytes.length,
                                    keyBytes.mutableBytes);
    assert(result == 0);
    _secKey = [FSRUtilities base64EncodedStringFromData:keyBytes];
    assert([_secKey length] == 24);

    CFHTTPMessageSetHeaderFieldValue(request, CFSTR("Upgrade"),
                                     CFSTR("websocket"));
    CFHTTPMessageSetHeaderFieldValue(request, CFSTR("Connection"),
                                     CFSTR("Upgrade"));
    if (_userAgent) {
        CFHTTPMessageSetHeaderFieldValue(request, CFSTR("User-Agent"),
                                         (__bridge CFStringRef)_userAgent);
    }

    if (_googleAppID) {
        CFHTTPMessageSetHeaderFieldValue(request, CFSTR("X-Firebase-GMPID"),
                                         (__bridge CFStringRef)_googleAppID);
    }

    CFHTTPMessageSetHeaderFieldValue(request, CFSTR("Sec-WebSocket-Key"),
                                     (__bridge CFStringRef)_secKey);
    CFHTTPMessageSetHeaderFieldValue(
        request, CFSTR("Sec-WebSocket-Version"),
        (__bridge CFStringRef)
        [NSString stringWithFormat:@"%u", (int)_webSocketVersion]);

    CFHTTPMessageSetHeaderFieldValue(request, CFSTR("Origin"),
                                     (__bridge CFStringRef)_url.SR_origin);

    if (_requestedProtocols) {
        CFHTTPMessageSetHeaderFieldValue(
            request, CFSTR("Sec-WebSocket-Protocol"),
            (__bridge CFStringRef)
    ...
    NSData *message =
        CFBridgingRelease(CFHTTPMessageCopySerializedMessage(request));
...
- (void)_initializeStreams;
{
    ...
    CFStreamCreatePairWithSocketToHost(NULL, (__bridge CFStringRef)host,
                                       (int)port, &readStream, &writeStream);
...
- (void)_connect;
{
    if (!_scheduledRunloops.count) {
        [self scheduleInRunLoop:[NSRunLoop FSR_networkRunLoop]
...
- (void)closeWithCode:(NSInteger)code reason:(NSString *)reason;
{
    ...
    dispatch_async(_workQueue, ^ {
        ...
        size_t maxMsgSize =
        [reason maximumLengthOfBytesUsingEncoding:NSUTF8StringEncoding];
        ...
        if (reason) {
            ...
            BOOL success = [reason
                            getBytes:(char *)mutablePayload.mutableBytes + sizeof(uint16_t)
            ...
            if (usedLength != maxMsgSize) {
                payload = [payload
                           subdataWithRange:NSMakeRange(0, usedLength + sizeof(uint16_t))];
            }
        }
...
- (void)_closeWithProtocolError:(NSString *)message;
{
    // Need to shunt this on the _callbackQueue first to see if they received any
    // messages
    [self _performDelegateBlock:^ {
...
{
    // Need to pingpong this off _callbackQueue first to make sure messages happen
    // in order
    [self _performDelegateBlock:^ {
             dispatch_async(self->_workQueue, ^{
...
- (void)handleCloseWithData:(NSData *)data;
{
    ...
    if (dataSize == 1) {
        ...
        [self _closeWithProtocolError:
              @"Payload for close must be larger than 2 bytes"];
        return;
    } else if (dataSize >= 2) {
        ...
        if (!closeCodeIsValid(_closeCode)) {
            [self _closeWithProtocolError:
                  [NSString stringWithFormat:@"Cannot have close code of %d",
                   _closeCode]];
            return;
        }
        if (dataSize > 2) {
            _closeReason = [[NSString alloc]
                            initWithData:[data subdataWithRange:NSMakeRange(2, dataSize - 2)]
                            encoding:NSUTF8StringEncoding];
            if (!_closeReason) {
...
- (void)_handleFrameWithData:(NSData *)frameData opCode:(NSInteger)opcode;
{
    ...
    BOOL isControlFrame = (opcode == SROpCodePing || opcode == SROpCodePong ||
                           opcode == SROpCodeConnectionClose);
    if (!isControlFrame) {
        [self _readFrameNew];
    } else {
        dispatch_async(_workQueue, ^ {
    ...
    case SROpCodeTextFrame: {
        NSString *str = [[NSString alloc] initWithData:frameData
                                          encoding:NSUTF8StringEncoding];
        if (str == nil && frameData) {
            [self closeWithCode:SRStatusCodeInvalidUTF8
                  reason:@"Text frames must be valid UTF-8"];
            dispatch_async(_workQueue, ^ {
                [self _disconnect];
            });
    ...
    default:
        [self
         _closeWithProtocolError:[NSString stringWithFormat:@"Unknown opcode %u",
                                  (int)opcode]];
...
- (void)_handleFrameHeader:(frame_header)frame_header curData:(NSData *)curData;
{
    ...
    BOOL isControlFrame = (frame_header.opcode == SROpCodePing ||
                           frame_header.opcode == SROpCodePong ||
    ...
    if (isControlFrame && frame_header.payload_length >= 126) {
        [self _closeWithProtocolError:
              @"Control frames cannot have payloads larger than 126 bytes"];
        return;
    }
    ...
    if (frame_header.payload_length == 0) {
        if (isControlFrame) {
            ...
            if (frame_header.fin) {
                [self _handleFrameWithData:_currentFrameData
        ...
        [self
         _addConsumerWithDataLength:(size_t)frame_header.payload_length
        callback:^(FSRWebSocket *self, NSData *newData) {
            if (isControlFrame) {
                [self _handleFrameWithData:newData
                 opCode:frame_header.opcode];
            } else {
                if (frame_header.fin) {
                    [self
                     _handleFrameWithData:self->_currentFrameData
                     opCode:frame_header.opcode];
                } else {
                    // TODO add assert that opcode is not a control;
                    [self _readFrameContinue];
...
- (void)_readFrameContinue;
{
    ...
    callback:^(FSRWebSocket *self, NSData *data) {
        ...
        if (headerBuffer[0] & SRRsvMask) {
            [self _closeWithProtocolError:
                  @"Server used RSV bits"];
            return;
        }

        uint8_t receivedOpcode =
            (SROpCodeMask & headerBuffer[0]);

        BOOL isControlFrame =
            (receivedOpcode == SROpCodePing ||
             receivedOpcode == SROpCodePong ||
             receivedOpcode == SROpCodeConnectionClose);

        if (!isControlFrame && receivedOpcode != 0 &&
                self->_currentFrameCount > 0) {
            [self _closeWithProtocolError:
                  @"all data frames after the initial data "
        ...
        if (receivedOpcode == 0 &&
                self->_currentFrameCount == 0) {
            [self _closeWithProtocolError:
                  @"cannot continue a message"];
            return;
        }

        header.opcode = receivedOpcode == 0
                        ? self->_currentFrameOpcode
                        : receivedOpcode;
        ...
        header.payload_length =
            SRPayloadLenMask & headerBuffer[1];

        headerBuffer = NULL;

        if (header.masked) {
            [self _closeWithProtocolError:
                  @"Client must receive unmasked data"];
        }

        size_t extra_bytes_needed =
            header.masked ? sizeof(self->_currentReadMaskKey)
            : 0;
        ...
        if (extra_bytes_needed == 0) {
            ...
            [self
             ...
             callback:^(FSRWebSocket *self,
            NSData *data) {
                size_t mapped_size =
                    data.length;
                const void *mapped_buffer =
                    data.bytes;
                size_t offset = 0;
                ...
                if (header.masked) {
                    assert(
                        mapped_size >=
                        sizeof(
                            self->_currentReadMaskOffset) +
                        offset);
                    memcpy(
                        self->_currentReadMaskKey,
                        ((uint8_t *)
                         mapped_buffer) +
                        offset,
                        sizeof(
...
- (void)_pumpWriting;
{
    ...
    if (dataLength - _outputBufferOffset > 0 && _outputStream.hasSpaceAvailable) {
        NSUInteger bytesWritten =
            [_outputStream write:_outputBuffer.bytes + _outputBufferOffset
        ...
        if (_outputBufferOffset > 4096 &&
                _outputBufferOffset > (_outputBuffer.length >> 1)) {
            _outputBuffer = [[NSMutableData alloc]
                             initWithBytes:(char *)_outputBuffer.bytes + _outputBufferOffset
    ...
    if (_closeWhenFinishedWriting &&
        ...
        @synchronized(self) {
            ...
            // TODO: Why are we missing the SocketRocket code to call
...
- (void)_scheduleCleanup {
    @synchronized(self) {
        ...
        // Cleanup NSStream delegate's in the same RunLoop used by the streams
        // themselves: This way we'll prevent race conditions between handleEvent
        // and SRWebsocket's dealloc
        NSTimer *timer =
            [NSTimer timerWithTimeInterval:(0.0f)
                     target:self
                     selector:@selector(_cleanupSelfReference:)
                     userInfo:nil
                     repeats:NO];
        [[NSRunLoop FSR_networkRunLoop] addTimer:timer
                                        forMode:NSDefaultRunLoopMode];
    }
}
...
- (void)_readUntilHeaderCompleteWithCallback:(data_callback)dataHandler;
{
    [self _readUntilBytes:CRLFCRLFBytes
          length:sizeof(CRLFCRLFBytes)
    ...
    callback:(data_callback)dataHandler;
{
    ...
    stream_scanner consumer = ^size_t(NSData *data) {
        ...
        for (int i = 0; i < size; i++) {
            if (((const unsigned char *)buffer)[i] ==
                    ((const unsigned char *)bytes)[match_count]) {
                match_count += 1;
...
- (BOOL)_innerPumpScanner {
    ...
    if (consumer.consumer) {
        NSData *tempView = [NSData
                            dataWithBytesNoCopy:(char *)_readBuffer.bytes + _readBufferOffset
    ...
    if (consumer.readToCurrentFrame || foundSize) {
        ...
        if (_readBufferOffset > 4096 &&
                _readBufferOffset > (_readBuffer.length >> 1)) {
            _readBuffer = [[NSMutableData alloc]
                           initWithBytes:(char *)_readBuffer.bytes + _readBufferOffset
                           length:_readBuffer.length - _readBufferOffset];
            _readBufferOffset = 0;
        }
        ...
        if (consumer.unmaskBytes) {
            ...
            for (int i = 0; i < len; i++) {
                bytes[i] = bytes[i] ^ _currentReadMaskKey[_currentReadMaskOffset %
        ...
        if (consumer.readToCurrentFrame) {
            ...
            if (_currentFrameOpcode == SROpCodeTextFrame) {
                ...
                if (_currentFrameOpcode == SROpCodeTextFrame && currentDataSize > 0) {
                    // TODO: Optimize the crap out of this.  Don't really have to copy all
                    // the data each time

                    size_t scanSize = currentDataSize - _currentStringScanPosition;

                    NSData *scan_data = [_currentFrameData
                                         subdataWithRange:NSMakeRange(_currentStringScanPosition,
                                                 scanSize)];
                    int32_t valid_utf8_size =
                        validate_dispatch_data_partial_string(scan_data);
...
- (void)_sendFrameWithOpcode:(FSROpCode)opcode data:(id)data;
{
    ...
    NSAssert([data isKindOfClass:[NSData class]] ||
             ...
             @"Function expects nil, NSString or NSData");
    ...
    NSMutableData *frame = [[NSMutableData alloc]
                            initWithLength:payloadLength + SRFrameHeaderOverhead];
    if (!frame) {
    ...
    if (payloadLength < 126) {
        ...
        *((uint16_t *)(frame_buffer + frame_buffer_size)) =
            EndianU16_BtoN((uint16_t)payloadLength);
        frame_buffer_size += sizeof(uint16_t);
    } else {
        frame_buffer[1] |= 127;
        *((uint64_t *)(frame_buffer + frame_buffer_size)) =
            EndianU64_BtoN((uint64_t)payloadLength);
    ...
    if (!useMask) {
        ...
        int result = SecRandomCopyBytes(kSecRandomDefault, sizeof(uint32_t),
                                        (uint8_t *)mask_key);
        assert(result == 0);
        ...
        for (int i = 0; i < payloadLength; i++) {
            frame_buffer[frame_buffer_size] =
                unmasked_payload[i] ^ mask_key[i % sizeof(uint32_t)];
            frame_buffer_size += 1;
        }
    }
...
- (void)stream:(NSStream *)aStream handleEvent:(NSStreamEvent)eventCode;
{
    ...
    if (eventCode == NSStreamEventOpenCompleted && aStream == _outputStream) {
        CFDataRef socketData = CFWriteStreamCopyProperty(
                                   (CFWriteStreamRef)_outputStream, kCFStreamPropertySocketNativeHandle);
        // In rare cases socketData might be nil (there are crash reports out
        // there), in which case we'll have to just live without keep-alive :(
        if (socketData != nil) {
            ...
            int keepAliveOn = 1;
            if (setsockopt(socket, SOL_SOCKET, SO_KEEPALIVE, &keepAliveOn,
                           sizeof(keepAliveOn)) == -1) {
    ...
    if (_secure && !_pinnedCertFound &&
        ...
        if (sslCerts) {
            SecTrustRef secTrust = (__bridge SecTrustRef)
                                   [aStream propertyForKey:(__bridge id)kCFStreamPropertySSLPeerTrust];
            if (secTrust) {
                ...
                for (NSInteger i = 0; i < numCerts && !_pinnedCertFound; i++) {
                    ...
                    for (id ref in sslCerts) {
                        ...
                        NSData *trustedCertData =
                            CFBridgingRelease(SecCertificateCopyData(trustedCert));
...
@{NSLocalizedDescriptionKey : @"Invalid server cert"};
                    [weakSelf
                     _failWithError:[NSError errorWithDomain:@"org.lolrus.SocketRocket"
                                     code:23556
                                     userInfo:userInfo]];
                });
                return;
...
- (void)safeHandleEvent:(NSStreamEvent)eventCode stream:(NSStream *)aStream {
    ...
    case NSStreamEventErrorOccurred: {
        // Note: The upstream code for SocketRocket logs the error message, but this
        // causes crashes on iOS 13
        // (https://github.com/firebase/firebase-ios-sdk/issues/3950)
        ...
        if (aStream.streamError) {
            ...
            dispatch_async(_workQueue, ^ {
                ...
                if (!self->_sentClose && !self->_failed) {
                    self->_sentClose = YES;
                    // If we get closed in this state it's probably not clean because we
                    // should be sending this when we send messages
                    [self _performDelegateBlock:^ {
                             if ([self.delegate respondsToSelector:@selector
                             (webSocket:didCloseWithCode:reason:wasClean:)]) {
                                 [self.delegate webSocket:self
...
{
    return [NSURLProtocol propertyForKey:@"FSR_SSLPinnedCertificates"
                          inRequest:self];
}
...
{
    [NSURLProtocol setProperty:FSR_SSLPinnedCertificates
                   forKey:@"FSR_SSLPinnedCertificates"
                   inRequest:self];
}
...
- (NSString *)SR_origin;
{
    ...
    if (self.port != nil) {
        return [NSString
                stringWithFormat:@"%@://%@:%@/", scheme, self.host, self.port];
    } else {
    ...
    if (codepoint == -1) {
        ...
        if (!U8_IS_LEAD(str[lastOffset]) ||
            ...
            for (int i = lastOffset + 1; i < offset; i++) {
                ...
                if (U8_IS_SINGLE(str[i]) || U8_IS_LEAD(str[i]) ||
                        !U8_IS_TRAIL(str[i])) {
                    size = -1;
                }
            }
...
/**
 * This is the main method of the thread on which the socket events are
 * scheduled in a run loop.
 */
- (void)main;
{
    @autoreleasepool {
        ...
        while ([_runLoop runMode:NSDefaultRunLoopMode
