    ...
    if (idx > 0) {
      if (comparator(keys[idx - 1], obj) != NSOrderedAscending) {
        [NSException raise:NSInvalidArgumentException
                    format:@"Can't create FImmutableSortedDictionary with keys "
                           @"with same ordering!"];
      }
    }
  }];
...
- (FImmutableSortedDictionary *)insertKey:(id)key withValue:(id)value {
  ...
  if (pos == NSNotFound) {
    ...
    if (self.count >= SORTED_DICTIONARY_ARRAY_TO_RB_TREE_SIZE_THRESHOLD) {
      ...
      return [[FArraySortedDictionary alloc] initWithComparator:self.comparator
                                                           keys:newKeys
                                                         values:newValues];
    }
  } else {
...
- (NSEnumerator *)reverseKeyEnumeratorFrom:(id)startKey {
  ...
  // *after* the closest match, but since this is a reverse iterator, we want to
  // start just *before* the closest match.
