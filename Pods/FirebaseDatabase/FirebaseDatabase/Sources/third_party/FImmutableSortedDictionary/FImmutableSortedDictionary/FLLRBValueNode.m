...
- (id)initWithKey:(__unsafe_unretained id)aKey
    withValue:(__unsafe_unretained id)aValue
    withColor:(__unsafe_unretained FLLRBColor *)aColor
    withLeft:(__unsafe_unretained id<FLLRBNode>)aLeft
    withRight:(__unsafe_unretained id<FLLRBNode>)aRight {
    self = [super init];
    if (self) {
        self.key = aKey;
        self.value = aValue;
...
- (id)copyWith:(__unsafe_unretained id)aKey
    withValue:(__unsafe_unretained id)aValue
    withColor:(__unsafe_unretained FLLRBColor *)aColor
    withLeft:(__unsafe_unretained id<FLLRBNode>)aLeft
    withRight:(__unsafe_unretained id<FLLRBNode>)aRight {
    return [[FLLRBValueNode alloc]
 ...
 * @return The first truthy value returned by action, or the last falsey value
 * returned by action.
 */
...
- (id<FLLRBNode>)insertKey:(__unsafe_unretained id)aKey
    forValue:(__unsafe_unretained id)aValue
    ...
    if (cmp == NSOrderedAscending) {
        n = [n copyWith:nil
               withValue:nil
               withColor:nil
               withLeft:[n.left insertKey:aKey
                       forValue:aValue
                       withComparator:aComparator]
               withRight:nil];
        ...
        n = [n copyWith:nil
               ...
               withColor:nil
               withLeft:nil
               withRight:[n.right insertKey:aKey
                        forValue:aValue
                        withComparator:aComparator]];
    }
...
- (id<FLLRBNode>)removeMin {
    ...
    n = [n copyWith:nil
           withValue:nil
           withColor:nil
           withLeft:[(FLLRBValueNode *)n.left removeMin]
           withRight:nil];
    return [n fixUp];
}
...
- (FLLRBValueNode *)moveRedLeft {
    ...
    if ([n.right.left isRed]) {
        n = [n copyWith:nil
               ...
               withColor:nil
               withLeft:nil
               withRight:[(FLLRBValueNode *)n.right rotateRight]];
        n = [n rotateLeft];
        n = [n colorFlip];
    }
    return n;
...
- (id<FLLRBNode>)rotateLeft {
    id<FLLRBNode> nl = [self copyWith:nil
                             withValue:nil
                             withColor:RED
                             withLeft:nil
                             withRight:self.right.left];
    return [self.right copyWith:nil
                       withValue:nil
...
- (id<FLLRBNode>)rotateRight {
    id<FLLRBNode> nr = [self copyWith:nil
                             withValue:nil
                             withColor:RED
                             withLeft:self.left.right
                             withRight:nil];
    return [self.left copyWith:nil
                      withValue:nil
                      withColor:self.color
                      withLeft:nil
                      withRight:nr];
...
- (id<FLLRBNode>)colorFlip {
    id<FLLRBNode> nleft =
        [self.left copyWith:nil
                   withValue:nil
                   withColor:[NSNumber numberWithBool:![self.left.color boolValue]]
                   withLeft:nil
                   withRight:nil];
    id<FLLRBNode> nright = [self.right
                            copyWith:nil
                            withValue:nil
                            withColor:[NSNumber numberWithBool:![self.right.color boolValue]]
                            withLeft:nil
                            withRight:nil];

    return [self copyWith:nil
                 withValue:nil
                 withColor:[NSNumber numberWithBool:![self.color boolValue]]
                 withLeft:nleft
                 withRight:nright];
}

- (id<FLLRBNode>)remove:(__unsafe_unretained id)aKey
        ...
        if (comparator(aKey, n.key) == NSOrderedSame) {
            if ([n.right isEmpty]) {
                ...
                n = [n copyWith:smallest.key
                       ...
                       withColor:nil
                       withLeft:nil
                       withRight:[(FLLRBValueNode *)n.right removeMin]];
            }
        }
        n = [n copyWith:nil
               withValue:nil
...
- (int)check {
    ...
    if (blackDepth != [self.right check]) {
        NSString *err = [NSString
                         stringWithFormat:@"(%@ -> %@)blackDepth: %d ; self.right check: %d",
                         self.value, [self.color boolValue] ? @"red" : @"black",
                         blackDepth, [self.right check]];
        //        return 10;
        @throw [[NSException alloc] initWithName:@"check" reason:err userInfo:nil];
    } else {
        int ret = blackDepth + ([self isRed] ? 0 : 1);
        //        NSLog(@"black depth is: %d; other is: %d, ret is: %d", blackDepth,
        //        ([self isRed] ? 0 : 1), ret);
        return ret;
    }
}
