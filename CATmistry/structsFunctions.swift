...
// MARK: - Arrays
private func getURLFromFilename(_ filename: String) -> URL {
    let url = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first!
    print("📂 App Group container path: \(url)")
    return url.appendingPathComponent(filename)
}

private func getPathFromFilename(_ filename: String) -> String {
    getURLFromFilename(filename).path
}

private func getDataFromJsonFile<T: Decodable>(filename: String, dataType _: T.Type) -> T? {
...
let learnTopics = getDataFromJsonFile(filename: "learnTopics.json", dataType: [Topic].self)!

let gameTopics = getDataFromJsonFile(filename: "gameTopics.json", dataType: [PlayTopic].self)!

let gasTestsGameArray = getDataFromJsonFile(filename: "gasTestsGameArray", dataType: [GasTestGame].self)!

let phGameArray = getDataFromJsonFile(filename: "phGameArray", dataType: [[PhGame]].self)!

let phGameOptionsArray = getDataFromJsonFile(filename: "phGameOptionsArray", dataType: [PhOption].self)!

let phGameAcidicUniversalStartColour = "monster-ph4.png"

let phGameAlkalinUniversalStartColour = "monster-ph10.png"

let perioicTable = getDataFromJsonFile(filename: "perioicTable", dataType: [[PeriodicTableGame]].self)!

let retrieveSolidsArray = getDataFromJsonFile(filename: "retrieveSolidsArray", dataType: [SpecificSeperationMethod].self)!
